...
class DataDefense(Defense):
    ...
    def exec(self, client_models, global_model, num_dps, net_portion, poisoned_train_loader_flag, device, *args, **kwargs):
        ...
        # calculate the loss on D_backdoor using trained model of each client i
        L_ac = []
        L_ap = []
        for net_idx, net in enumerate(client_models):
            net.eval()
            ...
            with torch.no_grad():
                for data in poisoned_train_loader_flag:
                    ...
                    for i in range(inputs.shape[0]):
                        if flag[i] == 1:
                            loss_ap += F.cross_entropy((outputs[i].view(1,-1)), targets[i].view(-1),reduction='none')
                            count_ap = count_ap+1
                        else:
                            loss_ac += F.cross_entropy(outputs[i].view(1,-1), targets[i].view(-1),reduction='none')
                            count_ac = count_ac+1
            ...
            L_ac.append(loss_ac.item())
            L_ap.append(loss_ap.item())
            del outputs, loss, loss_ac, loss_ap, inputs, targets, flag, gamma
            torch.cuda.empty_cache()

        print("\n#############################     CLEAN LOSS     ###################################")
        print(L_ac)
        print("Length of Losses = ",len(L_ac))
        print("\n#############################     POISON LOSS     ###################################")
        print(L_ap)
        print("Length of Losses = ",len(L_ap))

        # Calculate the norm of the clients and the global model
        vectorized_client_net = [vectorize_net(cm) for cm in client_models]
        vectorized_global_net = vectorize_net(global_model)
        vectorize_diff = [vectorized_client_net[i] - vectorized_global_net for i in range(len(client_models))]
        dist = [torch.norm(vectorize_diff[i]).item() for i in range(len(client_models))]

        print("\n################################     L2 NORM     ######################################")
        print(dist)
        print("Length of L2 norm btw clients and global model = ",len(dist))

        # Feature
        feature = [L_ac, L_ap, dist, net_portion]
        feature = np.array(feature, dtype='float32')
        feature = torch.from_numpy(feature).cuda()
        feature.requires_grad=True
        
        net_weight = torch.matmul(config.theta.clone(), feature.clone())

        print(net_weight.shape)
        net_weight = net_weight.squeeze(0)
        print(net_weight.shape)        

        print("\n#########################     CLIENT IMPORTANCE     ###########################")
        print("client importance: ")
        print(net_weight)
        
        # total_sum=1 but the values were -ve because of theta
        net_freq = torch.div(net_weight, torch.sum(net_weight))
        
        # Softmax : Range between 0 and 1 and their sum is equal to 1
        #net_weight = net_weight - torch.max(net_weight)
        #net_weight_normalize = torch.softmax(net_weight, dim=0)

        '''
        relu = nn.ReLU()
        net_weight_relu = relu(net_weight)
        print("relu on client importance = ",net_weight_relu)
        if torch.sum(net_weight_relu) != 0:
            net_weight_normalize = torch.div(net_weight_relu, torch.sum(net_weight_relu))
        else:
            net_weight_normalize = torch.div(net_weight_relu, (torch.sum(net_weight_relu)+0.0000001))
        '''

        print("Normalized client importance: ")
        print(net_freq)
        print("\n#######################################################################################")
