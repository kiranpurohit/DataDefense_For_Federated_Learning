import config
import numpy as np
import torch
import torch.nn.functional as F
import torch.nn as nn
from torch.autograd import grad
torch.autograd.set_detect_anomaly(True)

from utils import *
from defense import *

#from models.vgg import get_vgg_model
import models.vgg9 as vgg9

import pandas as pd

from torch.nn.utils import parameters_to_vector, vector_to_parameters
import datasets
torch.manual_seed(1)

import cvxpy as cp
from cvxpylayers.torch import CvxpyLayer
import random
import torchvision.models as models
import torchvision
import torchvision.transforms as transforms

#torch.set_deterministic()

class Net(nn.Module):
    def __init__(self, num_classes):
        super(Net, self).__init__()
        self.conv1 = nn.Conv2d(1, 32, 3, 1)
        self.conv2 = nn.Conv2d(32, 64, 3, 1)
        self.dropout1 = nn.Dropout2d(0.25)
        self.dropout2 = nn.Dropout2d(0.5)
        self.fc1 = nn.Linear(9216, 128)
        self.fc2 = nn.Linear(128, num_classes)

    def forward(self, x):
        x = self.conv1(x)
        x = F.relu(x)
        x = self.conv2(x)
        x = F.relu(x)
        x = F.max_pool2d(x, 2)
        x = self.dropout1(x)
        x = torch.flatten(x, 1)
        x = self.fc1(x)
        x = F.relu(x)
        x = self.dropout2(x)
        output = self.fc2(x)
        #output = F.log_softmax(x, dim=1)
        return output


def get_results_filename(poison_type, attack_method, model_replacement, project_frequency, defense_method, norm_bound, prox_attack, fixed_pool=False, model_arch="vgg9"):
    filename = "{}_{}_{}".format(poison_type, model_arch, attack_method)
    if fixed_pool:
        filename += "_fixed_pool" 
    
    if model_replacement:
        filename += "_with_replacement"
    else:
        filename += "_without_replacement"
    
    if attack_method == "pgd":
        filename += "_1_{}".format(project_frequency)
    
    if prox_attack:
        filename += "_prox_attack"

    if defense_method in ("norm-clipping", "norm-clipping-adaptive", "weak-dp"):
        filename += "_{}_m_{}".format(defense_method, norm_bound)
    elif defense_method in ("krum", "multi-krum", "rfa", "no-defense", "learning-defense"):
        filename += "_{}".format(defense_method)
               
    filename += "_acc_results.csv"

    return filename


def calc_norm_diff(gs_model, vanilla_model, epoch, fl_round, mode="bad"):
    norm_diff = 0
    for p_index, p in enumerate(gs_model.parameters()):
        norm_diff += torch.norm(list(gs_model.parameters())[p_index] - list(vanilla_model.parameters())[p_index]) ** 2
    norm_diff = torch.sqrt(norm_diff).item()
    if mode == "bad":
        #pdb.set_trace()
        logger.info("===> ND `|w_bad-w_g|` in local epoch: {} | FL round: {} |, is {}".format(epoch, fl_round, norm_diff))
    elif mode == "normal":
        logger.info("===> ND `|w_normal-w_g|` in local epoch: {} | FL round: {} |, is {}".format(epoch, fl_round, norm_diff))
    elif mode == "avg":
        logger.info("===> ND `|w_avg-w_g|` in local epoch: {} | FL round: {} |, is {}".format(epoch, fl_round, norm_diff))

    return norm_diff


def fed_avg_aggregator(net_list, net_freq, device, model="lenet"):
    #net list shape = 1
    #net freq shape = 1
    #net freq value = [1.0]
    if model == "lenet":
        net_avg = Net(num_classes=10).to(device)
    elif model in ("vgg9", "vgg11", "vgg13", "vgg16"):
        #net_avg = get_vgg_model(model).to(device)
        #net_avg = two_layer.Net1().to(device)
        net_avg = vgg9.Net1().to(device)
    whole_aggregator = []
    #count=0
    for p_index, p in enumerate(net_list[0].parameters()):
        #count = count+1
        params_aggregator = torch.zeros(p.size()).to(device)
        for net_index, net in enumerate(net_list):
            #print(net_index)----> 0
            #print(net_freq[net_index])-----> 1.0
            #we assume the adv model always comes to the beginning
            params_aggregator = params_aggregator + net_freq[net_index] * list(net.parameters())[p_index].data
        whole_aggregator.append(params_aggregator)
    #print(count)----> 18  (net_list.parameters)
    for param_index, p in enumerate(net_avg.parameters()):
        p.data = whole_aggregator[param_index]

    return net_avg


def estimate_wg(model, device, train_loader, optimizer, epoch, log_interval, criterion):
    logger.info("Prox-attack: Estimating wg_hat")
    model.train()
    for batch_idx, (data, target) in enumerate(train_loader):
        data, target = data.to(device), target.to(device)
        optimizer.zero_grad()
        output = model(data)
        loss = criterion(output, target)
        loss.backward()
        optimizer.step()
    
    if batch_idx % log_interval == 0:
            logger.info('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                epoch, batch_idx * len(data), len(train_loader.dataset),
                100. * batch_idx / len(train_loader), loss.item()))



def train(model, device, train_loader, optimizer, epoch, log_interval, criterion, pgd_attack=False, eps=5e-4, model_original=None,
        proj="l_2", project_frequency=1, adv_optimizer=None, prox_attack=False, wg_hat=None):
    """
        train function for both honest nodes and adversary.
        NOTE: this trains only for one epoch
    """
    model.train()
    # get learning rate
    for param_group in optimizer.param_groups:
        eta = param_group['lr']

    for batch_idx, (data, target) in enumerate(train_loader):
        data, target = data.to(device), target.to(device)

        #print("###############################")
        #print(target)

        optimizer.zero_grad()
        if pgd_attack:
            adv_optimizer.zero_grad()
        output = model(data)
        #loss = F.nll_loss(output, target)
        loss = criterion(output, target)
        if prox_attack:
            wg_hat_vec = parameters_to_vector(list(wg_hat.parameters()))
            model_vec = parameters_to_vector(list(model.parameters()))
            prox_term = torch.norm(wg_hat_vec - model_vec)**2
            loss = loss + prox_term
        
        loss.backward()
        if not pgd_attack:
            optimizer.step()
        else:
            if proj == "l_inf":
                w = list(model.parameters())
                n_layers = len(w)
                # adversarial learning rate
                eta = 0.001
                for i in range(len(w)):
                    # uncomment below line to restrict proj to some layers
                    if True:#i == 6 or i == 8 or i == 10 or i == 0 or i == 18:
                        w[i].data = w[i].data - eta * w[i].grad.data
                        # projection step
                        m1 = torch.lt(torch.sub(w[i], model_original[i]), -eps)
                        m2 = torch.gt(torch.sub(w[i], model_original[i]), eps)
                        w1 = (model_original[i] - eps) * m1
                        w2 = (model_original[i] + eps) * m2
                        w3 = (w[i]) * (~(m1+m2))
                        wf = w1+w2+w3
                        w[i].data = wf.data
            else:
                # do l2_projection
                adv_optimizer.step()
                w = list(model.parameters())
                w_vec = parameters_to_vector(w)
                model_original_vec = parameters_to_vector(model_original)
                # make sure you project on last iteration otherwise, high LR pushes you really far
                if (batch_idx%project_frequency == 0 or batch_idx == len(train_loader)-1) and (torch.norm(w_vec - model_original_vec) > eps):
                    # project back into norm ball
                    w_proj_vec = eps*(w_vec - model_original_vec)/torch.norm(
                            w_vec-model_original_vec) + model_original_vec
                    # plug w_proj back into model
                    vector_to_parameters(w_proj_vec, w)
                # for i in range(n_layers):
                #    # uncomment below line to restrict proj to some layers
                #    if True:#i == 16 or i == 17:
                #        w[i].data = w[i].data - eta * w[i].grad.data
                #        if torch.norm(w[i] - model_original[i]) > eps/n_layers:
                #            # project back to norm ball
                #            w_proj= (eps/n_layers)*(w[i]-model_original[i])/torch.norm(
                #                w[i]-model_original[i]) + model_original[i]
                #            w[i].data = w_proj

        if batch_idx % log_interval == 0:
            logger.info('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                epoch, batch_idx * len(data), len(train_loader.dataset),
                100. * batch_idx / len(train_loader), loss.item()))

class_main_task_acc= [[] for _ in range(10)]
#class_backdoor_acc= [[] for _ in range(10)]

def test(model, device, test_loader, test_batch_size, criterion, mode="raw-task", dataset="cifar10", poison_type="fashion"):
    class_correct = list(0. for i in range(10))
    class_total = list(0. for i in range(10))
    
    if dataset in ("mnist", "emnist"):
        target_class = 7
        if mode == "raw-task":
            classes = [str(i) for i in range(10)]
        elif mode == "targetted-task":
            if poison_type == 'ardis':
                classes = [str(i) for i in range(10)]
            else: 
                classes = ["T-shirt/top", 
                            "Trouser",
                            "Pullover",
                            "Dress",
                            "Coat",
                            "Sandal",
                            "Shirt",
                            "Sneaker",
                            "Bag",
                            "Ankle boot"]
    elif dataset == "cifar10":
        classes = ('plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck')
        # target_class = 2 for greencar, 9 for southwest
        if poison_type in ("howto", "greencar-neo"):
            target_class = 2
        else:
            target_class = 9

    elif dataset == "triggerpatch":
        classes = ('plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck')
        target_class = 2

    model.eval()
    test_loss = 0
    correct = 0
    backdoor_correct = 0
    backdoor_tot = 0
    final_acc = 0
    task_acc = None

    with torch.no_grad():
        for data, target in test_loader:

            #target=target.long()

            data, target = data.to(device), target.to(device)
            output = model(data)
            _, predicted = torch.max(output, 1)
            c = (predicted == target).squeeze()

            #test_loss += F.nll_loss(output, target, reduction='sum').item()  # sum up batch loss
            test_loss += criterion(output, target).item()
            pred = output.argmax(dim=1, keepdim=True)  # get the index of the max log-probability
            correct += pred.eq(target.view_as(pred)).sum().item()
            # check backdoor accuracy
            if poison_type == 'ardis':
                backdoor_index = torch.where(target == target_class)
                target_backdoor = torch.ones_like(target[backdoor_index])
                predicted_backdoor = predicted[backdoor_index]
                backdoor_correct += (predicted_backdoor == target_backdoor).sum().item()
                backdoor_tot = backdoor_index[0].shape[0]
                # logger.info("Target: {}".format(target_backdoor))
                # logger.info("Predicted: {}".format(predicted_backdoor))

            #for image_index in range(test_batch_size):
            for image_index in range(len(target)):
                label = target[image_index]
                class_correct[label] += c[image_index].item()
                class_total[label] += 1
    test_loss /= len(test_loader.dataset)

    if mode == "raw-task":
        for i in range(10):
            logger.info('Accuracy of %5s : %.2f %%' % (
                classes[i], 100 * class_correct[i] / class_total[i]))

            if i == target_class:
                task_acc = 100 * class_correct[i] / class_total[i]

        logger.info('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.2f}%)\n'.format(
            test_loss, correct, len(test_loader.dataset),
            100. * correct / len(test_loader.dataset)))
        final_acc = 100. * correct / len(test_loader.dataset)


    if mode == "raw-task_acc":
        for i in range(10):
            logger.info('Accuracy of %5s : %.2f %%' % (
                classes[i], 100 * class_correct[i] / class_total[i]))
            class_main_task_acc[i].append(100 * class_correct[i] / class_total[i])
            if i == target_class:
                task_acc = 100 * class_correct[i] / class_total[i]

        logger.info('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.2f}%)\n'.format(
            test_loss, correct, len(test_loader.dataset),
            100. * correct / len(test_loader.dataset)))
        final_acc = 100. * correct / len(test_loader.dataset)


    elif mode == "targetted-task":

        if dataset in ("mnist", "emnist"):
            for i in range(10):
                logger.info('Accuracy of %5s : %.2f %%' % (
                    classes[i], 100 * class_correct[i] / class_total[i]))
            if poison_type == 'ardis':
                # ensure 7 is being classified as 1
                logger.info('Backdoor Accuracy of %.2f : %.2f %%' % (
                     target_class, 100 * backdoor_correct / backdoor_tot))
                final_acc = 100 * backdoor_correct / backdoor_tot
            else:
                # trouser acc
                final_acc = 100 * class_correct[1] / class_total[1]
        
        elif dataset in ("cifar10","triggerpatch"):

            logger.info('#### Targetted Accuracy of %5s : %.2f %%' % (classes[target_class], 100 * class_correct[target_class] / class_total[target_class]))
            final_acc = 100 * class_correct[target_class] / class_total[target_class]
            '''
            for i in range(10):
                logger.info('Accuracy of %5s : %.2f %%' % (classes[i], 100 * class_correct[i] / class_total[i]))
                class_main_task_acc[i].append(100 * class_correct[i] / class_total[i])

            logger.info('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.2f}%)\n'.format(
                test_loss, correct, len(test_loader.dataset), 100. * correct / len(test_loader.dataset)))
            final_acc = 100. * correct / len(test_loader.dataset)
            '''
    return final_acc, task_acc

class FederatedLearningTrainer:
    def __init__(self, *args, **kwargs):
        self.hyper_params = None

    def run(self, client_model, *args, **kwargs):
        raise NotImplementedError()


class FrequencyFederatedLearningTrainer(FederatedLearningTrainer):
    def __init__(self, arguments=None, *args, **kwargs):
        #self.poisoned_emnist_dataset = arguments['poisoned_emnist_dataset']
        self.vanilla_model = arguments['vanilla_model']
        self.net_avg = arguments['net_avg']
        self.net = arguments['net']
        self.net_dataidx_map = arguments['net_dataidx_map']
        self.num_nets = arguments['num_nets']
        self.part_nets_per_round = arguments['part_nets_per_round']
        self.fl_round = arguments['fl_round']
        self.local_training_period = arguments['local_training_period']
        self.adversarial_local_training_period = arguments['adversarial_local_training_period']
        self.args_lr = arguments['args_lr']
        self.args_gamma = arguments['args_gamma']
        self.attacking_fl_rounds = arguments['attacking_fl_rounds']
        self.poisoned_emnist_train_loader = arguments['poisoned_emnist_train_loader']
        self.clean_train_loader = arguments['clean_train_loader']
        self.mislabeled_train_loader = arguments['mislabeled_train_loader']
        self.poisoned_trainset = arguments['poisoned_trainset']          
        self.poisoned_train_loader_1 = arguments['poisoned_train_loader_1']
        self.vanilla_emnist_test_loader = arguments['vanilla_emnist_test_loader']
        self.targetted_task_test_loader = arguments['targetted_task_test_loader']
        self.batch_size = arguments['batch_size']
        self.test_batch_size = arguments['test_batch_size']
        self.log_interval = arguments['log_interval']
        self.device = arguments['device']
        self.num_dps_poisoned_dataset = arguments['num_dps_poisoned_dataset']
        self.defense_technique = arguments["defense_technique"]
        self.norm_bound = arguments["norm_bound"]
        self.attack_method = arguments["attack_method"]
        self.dataset = arguments["dataset"]
        self.model = arguments["model"]
        self.criterion = nn.CrossEntropyLoss()
        self.eps = arguments['eps']
        self.poison_type = arguments['poison_type']
        self.model_replacement = arguments['model_replacement']
        self.project_frequency = arguments['project_frequency']
        self.adv_lr = arguments['adv_lr']
        self.prox_attack = arguments['prox_attack']
        self.attack_case = arguments['attack_case']
        self.stddev = arguments['stddev']
        self.loss_clean = arguments['loss_clean']
        self.loss_poison = arguments['loss_poison']
        self.loss_total = arguments['loss_total']
        self.value_total = arguments['value_total']
        self.gamma_lr = arguments['gamma_lr']

        logger.info("Posion type! {}".format(self.poison_type))

        if self.poison_type == 'ardis':
            self.ardis_dataset = datasets.get_ardis_dataset()
            # exclude first 66 points because they are part of the adversary
            if self.attack_case == 'normal-case':
                self.ardis_dataset.data = self.ardis_dataset.data[66:]
            elif self.attack_case == 'almost-edge-case':
                self.ardis_dataset.data = self.ardis_dataset.data[66:132]
                
        elif self.poison_type == 'southwest':
            # It will return none
            self.ardis_dataset = datasets.get_southwest_dataset(attack_case=self.attack_case)
        else:
            self.ardis_dataset=None


        if self.attack_method == "pgd":
            self.pgd_attack = True
        else:
            self.pgd_attack = False

        if arguments["defense_technique"] == "no-defense":
            self._defender = None
        elif arguments["defense_technique"] == "norm-clipping" or arguments["defense_technique"] == "norm-clipping-adaptive":
            self._defender = WeightDiffClippingDefense(norm_bound=arguments['norm_bound'])
        elif arguments["defense_technique"] == "weak-dp":
            # doesn't really add noise. just clips
            self._defender = WeightDiffClippingDefense(norm_bound=arguments['norm_bound'])
        ############################################################
        elif arguments["defense_technique"] == "data-defense":
            self._defender = DataDefense()
        ############################################################
        elif arguments["defense_technique"] == "krum":
            self._defender = Krum(mode='krum', num_workers=self.part_nets_per_round, num_adv=1)
        elif arguments["defense_technique"] == "multi-krum":
            self._defender = Krum(mode='multi-krum', num_workers=self.part_nets_per_round, num_adv=1)
        elif arguments["defense_technique"] == "rfa":
            self._defender = RFA()
        else:
            NotImplementedError("Unsupported defense method !")
        #print(type(self._defender))

    def run(self):
        main_task_acc = []
        raw_task_acc = []
        backdoor_task_acc = []
        fl_iter_list = []
        adv_norm_diff_list = []
        wg_norm_list = []
        freq_list = []
        poison_clean_indices = []
        gamma_values = []
        gamma_diff1 = []
        ce_loss = []
        red_top = []
        green_top = []
        red_rest = []
        green_rest = []
        psi_norm = []

        # let's conduct multi-round training
        for flr in range(1, self.fl_round+1):
            logger.info("##### attack fl rounds: {}".format(self.attacking_fl_rounds))
            g_user_indices = []

            if self.defense_technique == "norm-clipping-adaptive":
                # experimental
                norm_diff_collector = []

            if flr in self.attacking_fl_rounds:
                # randomly select participating clients
                # in this current version, we sample `part_nets_per_round-1` per FL round since we assume attacker will                   always participates
                # 1 attacker and 9 clients (select 9 clients randomly out of 200 clients)
                # 9 selected client indices
                selected_node_indices = np.random.choice(self.num_nets, size=self.part_nets_per_round-1, replace=False)
                
                # each selected client with its number of images
                num_data_points = [len(self.net_dataidx_map[i]) for i in selected_node_indices]
                
                # total number of images in each round (sum of 1 attacker having (500 images) and 9 clients images)
                total_num_dps_per_round = sum(num_data_points) + self.num_dps_poisoned_dataset

                logger.info("FL round: {}, total num data points: {}, num dps poisoned: {}".format(flr, num_data_points, self.num_dps_poisoned_dataset))

                # each 10 users(1 attacker and 9 clients) frequency (its no. of images/total no. of images in that round)
                net_freq = [self.num_dps_poisoned_dataset/ total_num_dps_per_round] + [num_data_points[i]/total_num_dps_per_round for i in range(self.part_nets_per_round-1)]
                
                logger.info("Net freq: {}, FL round: {} with adversary".format(net_freq, flr)) 
                #pdb.set_trace()

                # we need to reconstruct the net list at the beginning
                # a list of len 10 having same global model
                net_list = [copy.deepcopy(self.net_avg) for _ in range(self.part_nets_per_round)]
                #print(len(net_list))---> 10
                
                logger.info("################## Starting fl round: {}".format(flr))
                
                # original model's parameters
                model_original = list(self.net_avg.parameters())
                #print(len(model_original))----> 18
                
                # super hacky but I'm doing this for the prox-attack
                wg_clone = copy.deepcopy(self.net_avg)
                wg_hat = None
                
                # original model's parameters to vector
                v0 = torch.nn.utils.parameters_to_vector(model_original)
                #print(len(v0))---> 9225610
                #print(v0)--> tensor([ 0.4773, -0.1033, -0.3494,  ..., -0.0628, -0.0384, -0.2275]
                
                # norm of v0 which is a vector
                # wg_norm_list is of len 500 (fl-rounds) and it contains the norm of the global model 
                wg_norm_list.append(torch.norm(v0).item())
                #print(len(wg_norm_list))----> 1
                #print(wg_norm_list)----> [37.29024124145508]
                
                # start the FL process
                # It will run 10 times as net_list is of len 10, net_idx means the index and net means the global model
                for net_idx, net in enumerate(net_list):
                    if net_idx == 0:
                        global_user_idx = -1 # we assign "-1" as the indices of the attacker in global user indices
                        pass
                    else:
                        global_user_idx = selected_node_indices[net_idx-1]
                        dataidxs = self.net_dataidx_map[global_user_idx]
                        if self.attack_case == "edge-case":
                            # get a local train dataloader for each clients images 
                            train_dl_local, _ = get_dataloader(self.dataset, './data', self.batch_size, 
                                                            self.test_batch_size, dataidxs) # also get the data loader
                        elif self.attack_case in ("normal-case", "almost-edge-case"):
                            train_dl_local, _ = get_dataloader_normal_case(self.dataset, './data', self.batch_size, 
                                                            self.test_batch_size, dataidxs, user_id=global_user_idx,
                                                            num_total_users=self.num_nets,
                                                            poison_type=self.poison_type,
                                                            ardis_dataset=self.ardis_dataset,
                                                            attack_case=self.attack_case) # also get the data loader
                        else:
                            NotImplementedError("Unsupported attack case ...")

                    g_user_indices.append(global_user_idx)
                    if net_idx == 0:
                        logger.info("@@@@@@@@ Working on client: {}, which is Attacker".format(net_idx))
                    else:
                        logger.info("@@@@@@@@ Working on client: {}, which is Global user: {}".format(net_idx, global_user_idx))

                    criterion = nn.CrossEntropyLoss()
                    
                    # net is the global model which gets replaced by net_avg in every fl round
                    # clients have their sampled images with them and they train their images on the global model only 
                    optimizer = optim.SGD(net.parameters(), lr=self.args_lr*self.args_gamma**(flr-1), momentum=0.9, weight_decay=1e-4) # epoch, net, train_loader, optimizer, criterion
                    
                    adv_optimizer = optim.SGD(net.parameters(), lr=self.adv_lr*self.args_gamma**(flr-1), momentum=0.9, weight_decay=1e-4) # looks like adversary needs same lr to hide with others
                    
                    # wg_clone is net_avg only
                    prox_optimizer = optim.SGD(wg_clone.parameters(), lr=self.args_lr*self.args_gamma**(flr-1), momentum=0.9, weight_decay=1e-4)
                    for param_group in optimizer.param_groups:
                        logger.info("Effective lr in FL round: {} is {}".format(flr, param_group['lr']))
                    
                    if net_idx == 0:
                        # attacker in the attacking round 
                        if self.prox_attack:
                            # estimate w_hat
                            for inner_epoch in range(1, self.local_training_period+1):
                                estimate_wg(wg_clone, self.device, self.clean_train_loader, prox_optimizer, inner_epoch, log_interval=self.log_interval, criterion=self.criterion)
                            wg_hat = wg_clone

                        for e in range(1, self.adversarial_local_training_period+1):
                           # we always assume net_index 0 is adversary
                           
                            if self.defense_technique in ('krum', 'multi-krum'):
                                #print("Attacker")
                                # train on 500 images (100+400) of attacker as it is a attacker in a attacking round
                                # train with eps = 2 * 0.998 **(flr-1)
                                train(net, self.device, self.poisoned_emnist_train_loader, optimizer, e, log_interval=self.log_interval, criterion=self.criterion,pgd_attack=self.pgd_attack, eps=self.eps*self.args_gamma**(flr-1), model_original=model_original, project_frequency=self.project_frequency, adv_optimizer=adv_optimizer, prox_attack=self.prox_attack, wg_hat=wg_hat)
                                
                            else:
                                # train with eps = 2
                                #print("Attacker")
                                train(net, self.device, self.poisoned_emnist_train_loader, optimizer, e, log_interval=self.log_interval, criterion=self.criterion, pgd_attack=self.pgd_attack, eps=self.eps, model_original=model_original, project_frequency=self.project_frequency, adv_optimizer=adv_optimizer, prox_attack=self.prox_attack, wg_hat=wg_hat)

                        # test on 10000 test images cifar10
                        test(net, self.device, self.vanilla_emnist_test_loader, test_batch_size=self.test_batch_size, criterion=self.criterion, mode="raw-task", dataset=self.dataset, poison_type=self.poison_type)
                        
                        # test on 196 southwest test images
                        test(net, self.device, self.targetted_task_test_loader, test_batch_size=self.test_batch_size, criterion=self.criterion, mode="targetted-task", dataset=self.dataset, poison_type=self.poison_type)

                        # if model_replacement scale models
                        if self.model_replacement:
                            v = torch.nn.utils.parameters_to_vector(net.parameters())
                            logger.info("Attacker before scaling : Norm = {}".format(torch.norm(v)))
                            # adv_norm_diff = calc_norm_diff(gs_model=net, vanilla_model=self.net_avg, epoch=e, fl_round=flr, mode="bad")
                            # logger.info("||w_bad - w_avg|| before scaling = {}".format(adv_norm_diff))

                            for idx, param in enumerate(net.parameters()):
                                param.data = (param.data - model_original[idx])*(total_num_dps_per_round/self.num_dps_poisoned_dataset) + model_original[idx]
                            v = torch.nn.utils.parameters_to_vector(net.parameters())
                            logger.info("Attacker after scaling : Norm = {}".format(torch.norm(v)))

                        # at here we can check the distance between w_bad and w_g i.e. `\|w_bad - w_g\|_2`
                        # we can print the norm diff out for debugging
                        adv_norm_diff = calc_norm_diff(gs_model=net, vanilla_model=self.net_avg, epoch=e, fl_round=flr,mode= "bad")
                        # adv_norm_diff_list is of len 500 (fl-rounds) and it contains norm diff of adv and global model
                        # if it is a attacking round it will contain a value if not a attacking round, it will be 0
                        adv_norm_diff_list.append(adv_norm_diff)

                        if self.defense_technique == "norm-clipping-adaptive":
                            # experimental
                            norm_diff_collector.append(adv_norm_diff)
                            
                    
                    
                    # for other 9 clients which are honest in a attacking round
                    else:
                        for e in range(1, self.local_training_period+1):
                           # net is the global model broadcasted to each client on which clients do their local training
                           # train_dl_local is the client's sampled images dataloader
                           #print("Client", net_idx)
                           train(net, self.device, train_dl_local, optimizer, e, log_interval=self.log_interval, criterion=self.criterion)                
                           # at here we can check the distance between w_normal and w_g i.e. `\|w_bad - w_g\|_2`
                        # we can print the norm diff out for debugging
                        honest_norm_diff = calc_norm_diff(gs_model=net, vanilla_model=self.net_avg, epoch=e, fl_round=flr, mode="normal")
                        
                        if self.defense_technique == "norm-clipping-adaptive":
                            # experimental
                            norm_diff_collector.append(honest_norm_diff)     
                 
                 
                            
                            
            # Non attacking rounds where all 10 clients are honest
            else:
                # select randomly 10 clients out of 200 clients
                selected_node_indices = np.random.choice(self.num_nets, size=self.part_nets_per_round, replace=False)
                # number of sampled images of each client
                num_data_points = [len(self.net_dataidx_map[i]) for i in selected_node_indices]
                total_num_dps_per_round = sum(num_data_points)
                
                # net_freq = client's no. of images/total no. of images in that fl round
                net_freq = [num_data_points[i]/total_num_dps_per_round for i in range(self.part_nets_per_round)]
                logger.info("Net freq: {}, FL round: {} without adversary".format(net_freq, flr))

                # we need to reconstruct the net list at the beginning
                # net_list is of len 10 each containing global model
                net_list = [copy.deepcopy(self.net_avg) for _ in range(self.part_nets_per_round)]
                logger.info("################## Starting fl round: {}".format(flr))
                
                
                # start the FL process
                # It will run 10 times as net_list is of len 10, net_idx is the index and net is the global model
                for net_idx, net in enumerate(net_list):
                    # global index of each selected client in a fl round
                    global_user_idx = selected_node_indices[net_idx]
                    # index of the sampled images of each client in a fl round
                    dataidxs = self.net_dataidx_map[global_user_idx]
                    
                    # get a local train dataloader for each client's sampled images 
                    if self.attack_case == "edge-case":
                        train_dl_local, _ = get_dataloader(self.dataset, './data', self.batch_size, 
                                                       self.test_batch_size, dataidxs) # also get the data loader
                    elif self.attack_case in ("normal-case", "almost-edge-case"):
                        train_dl_local, _ = get_dataloader_normal_case(self.dataset, './data', self.batch_size, 
                                                            self.test_batch_size, dataidxs, user_id=global_user_idx,
                                                            num_total_users=self.num_nets,
                                                            poison_type=self.poison_type,
                                                            ardis_dataset=self.ardis_dataset,
                                                            attack_case=self.attack_case) # also get the data loader
                    else:
                        NotImplementedError("Unsupported attack case ...")
                    
                    # g_user_indices contains 10 global clients selected in this fl round
                    g_user_indices.append(global_user_idx)
                    
                    logger.info("@@@@@@@@ Working on client: {}, which is Global user: {}".format(net_idx, global_user_idx))

                    criterion = nn.CrossEntropyLoss()
                    optimizer = optim.SGD(net.parameters(), lr=self.args_lr*self.args_gamma**(flr-1), momentum=0.9, weight_decay=1e-4) # epoch, net, train_loader, optimizer, criterion
                    for param_group in optimizer.param_groups:
                        logger.info("Effective lr in FL round: {} is {}".format(flr, param_group['lr']))

                    for e in range(1, self.local_training_period+1):
                        #print("client", net_idx)
                        train(net, self.device, train_dl_local, optimizer, e, log_interval=self.log_interval, criterion=self.criterion)

                    honest_norm_diff = calc_norm_diff(gs_model=net, vanilla_model=self.net_avg, epoch=e, fl_round=flr, mode="normal")

                    if self.defense_technique == "norm-clipping-adaptive":
                        # experimental
                        norm_diff_collector.append(honest_norm_diff)

                # adv_norm_diff_list is of len 500 (fl-rounds) and it contains norm diff of adv and global model
                # It is a non attacking round so here no attacker will be there hence adv_norm_diff is 0
                adv_norm_diff_list.append(0)
                model_original = list(self.net_avg.parameters())
                v0 = torch.nn.utils.parameters_to_vector(model_original)
                # wg_norm_list is of len 500 (fl-rounds) and it contains the norm of the global model
                wg_norm_list.append(torch.norm(v0).item())


            # conduct defense here:
            # after all 10 workers execution in a fl round, then defense will take place
            ##############################################################################

            if self.defense_technique == "data-defense":

                if (flr % 300) == 1:
                    self.gamma_lr = self.gamma_lr*0.1
                    print("gamma_lr = ",self.gamma_lr)

                # Random 100 indices out of 400 clean images are called D_clean
                if flr==1:

                    #D_clean = np.random.choice(400, 100, replace=False)

                    D_correct = np.random.choice(400, 85, replace=False)
                    poison = np.arange(400,500)
                    D_wrong = np.random.choice(poison, 15, replace=False)
                    D_clean = np.append(D_correct, D_wrong)


                    D_clean = np.sort(D_clean)

                    sampled_100_indices = D_clean

                    indices_500 = np.arange(500)
                    #indices_500 = np.arange(10)

                    sampled_400_indices = list(set(indices_500) - set(sampled_100_indices))
                    ########################################################################
                    print(sampled_100_indices)
                    print(sampled_400_indices)
                    ########################################################################

                    for epoch in range(100):

                        for inputs,labels in self.poisoned_train_loader_1:
                            inputs, targets = inputs.cuda(), labels.cuda()
                            out = self.net(inputs)
                            feature_500 = out.view(out.size(0), -1)

                        print(feature_500.shape) #should be no. of images, each of 512 dimension

                        feature_100_clean = feature_500[sampled_100_indices]
                        feature_400_not_known = feature_500[sampled_400_indices]

                        # ***************************************************************************************************************

                        #print("w1",config.w1)
                        #print("w2",config.w2)
                        #print("w3",config.w3)
                        #print("w4",config.w4)

                        net_100_multiply1 = torch.matmul(config.w1.clone(), torch.t(feature_100_clean).clone())
                        net_400_multiply1 = torch.matmul(config.w1.clone(), torch.t(feature_400_not_known).clone())

                        print("net_100_multiply1",net_100_multiply1)
                        print("net_400_multiply1",net_400_multiply1)

                        relu = nn.ReLU()
                        relu_net_100_multiply1 = relu(net_100_multiply1)
                        relu_net_400_multiply1 = relu(net_400_multiply1)

                        net_100_multiply2 = torch.matmul(config.w2.clone(), relu_net_100_multiply1.clone())
                        net_400_multiply2 = torch.matmul(config.w2.clone(), relu_net_400_multiply1.clone())

                        net_100_multiply2 = torch.t(net_100_multiply2)
                        net_400_multiply2 = torch.t(net_400_multiply2)

                        #print("net_100_multiply2 shape sould be (10,clean)",net_100_multiply2.shape)
                        '''
                        logsoftmax = nn.LogSoftmax(dim=1)
                        softmax_net_100_multiply2 = logsoftmax(net_100_multiply2)
                        softmax_net_400_multiply2 = logsoftmax(net_400_multiply2)
                        '''
                        softmax_net_100_multiply2 = torch.softmax(net_100_multiply2, dim=1)
                        softmax_net_400_multiply2 = torch.softmax(net_400_multiply2, dim=1)

                        print("net_100_multiply2",net_100_multiply2)
                        print("net_400_multiply2",net_400_multiply2)
                        print("softmax_net_100_multiply2",softmax_net_100_multiply2)
                        print("softmax_net_400_multiply2",softmax_net_400_multiply2)

                        # Check 2: Check the shape of output of softmax
                        #print("softmax shape sould be (clean,10)",softmax_net_100_multiply2.shape)

                        # Check 3: Find one hot vector of y labels of both datasets

                        #############################################################################################
                        #y_one_hot_100 = torch.zeros((2, 10), requires_grad=False).cuda()
                        #y_one_hot_400 = torch.zeros((8, 10), requires_grad=False).cuda()
                        #y_one_hot_100[torch.arange(2), self.poisoned_trainset.targets[394:396]] = 1
                        #y_one_hot_400[torch.arange(8), self.poisoned_trainset.targets[396:404]] = 1

                        y_one_hot_100 = torch.zeros((100, 10), requires_grad=False).cuda()
                        y_one_hot_400 = torch.zeros((400, 10), requires_grad=False).cuda()
                        y_one_hot_100[torch.arange(100), self.poisoned_trainset.targets[sampled_100_indices]] = 1
                        y_one_hot_400[torch.arange(400), self.poisoned_trainset.targets[sampled_400_indices]] = 1
                        #############################################################################################

                        print("y one hot 100 output", y_one_hot_100)
                        print("y one hot 400 output", y_one_hot_400)

                        ###########################################################################
                        #clean_targets = self.poisoned_trainset.targets[394:396]
                        clean_targets = self.poisoned_trainset.targets[sampled_100_indices]

                        clean_targets = torch.from_numpy(clean_targets).cuda()
                        clean_targets = clean_targets.long()
                        cross_loss = nn.CrossEntropyLoss()
                        ce_clean_loss = cross_loss(softmax_net_100_multiply2, clean_targets)
                        ce_loss.append(ce_clean_loss.item())
                        ###########################################################################

                        concat_pred_given_100 = torch.cat((softmax_net_100_multiply2, y_one_hot_100), 1)
                        concat_pred_given_400 = torch.cat((softmax_net_400_multiply2, y_one_hot_400), 1)

                        net_100_multiply3 = torch.matmul(config.w3.clone(), torch.t(concat_pred_given_100).clone())
                        net_400_multiply3 = torch.matmul(config.w3.clone(), torch.t(concat_pred_given_400).clone())

                        relu_net_100_multiply3 = relu(net_100_multiply3)
                        relu_net_400_multiply3 = relu(net_400_multiply3)

                        print("net_100_multiply3",net_100_multiply3)
                        print("net_400_multiply3",net_400_multiply3)
                        print("relu_net_100_multiply3",relu_net_100_multiply3)
                        print("relu_net_400_multiply3",relu_net_400_multiply3)

                        net_100_multiply4 = torch.matmul(config.w4.clone(), relu_net_100_multiply3.clone())
                        net_400_multiply4 = torch.matmul(config.w4.clone(), relu_net_400_multiply3.clone())

                        print("net_100_multiply4",net_100_multiply4)
                        print("net_400_multiply4",net_400_multiply4)

                        max_100 = torch.max(net_100_multiply4)
                        max_400 = torch.max(net_400_multiply4)
                        maxi = torch.max(max_100 , max_400)

                        min_100 = torch.min(net_100_multiply4)
                        min_400 = torch.min(net_400_multiply4)
                        mini = torch.min(min_100, min_400)

                        print("max_100",max_100)
                        print("max_400",max_400)
                        print("min_100",min_100)
                        print("min_400",min_400)

                        if maxi !=0 or mini!=0:
                            gamma_100 = torch.div(torch.sub(net_100_multiply4,mini) , torch.sub(maxi,mini))
                            gamma_400 = torch.div(torch.sub(net_400_multiply4,mini) , torch.sub(maxi,mini))
                        else:
                            gamma_100 = torch.div(torch.sub(net_100_multiply4,mini) , torch.sub(maxi,mini)+0.0000001)
                            gamma_400 = torch.div(torch.sub(net_400_multiply4,mini) , torch.sub(maxi,mini)+0.0000001)

                        print("maxi",maxi)
                        print("mini",mini)

                        print("gamma_100",gamma_100)
                        print("gamma_400",gamma_400)

                        gamma_100 = gamma_100.squeeze(0)
                        gamma_400 = gamma_400.squeeze(0)

                        print(gamma_100.shape , gamma_400.shape)

                        diff_gamma = gamma_100.unsqueeze(0)-gamma_400.unsqueeze(1)
                        print("diff_gamma",diff_gamma)

                        #####################################################
                        new_loss = diff_gamma.sum() + 1000*ce_clean_loss
                        new_loss.backward(retain_graph=True)
                        #####################################################

                        gamma_diff1.append(diff_gamma.sum().item())

                        with torch.no_grad():

                            for param in self.net.parameters():
                                param = param - 0.00001 * param.grad
                            for param in self.net.parameters():
                                param.grad.zero_()

                            #print("\n w1, w2, w3, w4 before gradient descent = ",config.w1, config.w2, config.w3, config.w4)
                            #print("\n Grad of gammadiff wrt w1, w2, w3, w4 = ","\n\n", config.w1.grad,"\n\n",config.w2.grad,"\n\n", config.w3.grad,"\n\n", config.w4.grad)
                            config.w1.sub_(config.w1.grad*0.00001)
                            config.w2.sub_(config.w2.grad*0.00001)
                            config.w3.sub_(config.w3.grad*0.00001)
                            config.w4.sub_(config.w4.grad*0.00001)
                            #print("\n w1, w2, w3, w4 after gradient descent = ",config.w1, config.w2, config.w3, config.w4)

                            config.w1.grad.zero_()
                            config.w2.grad.zero_()
                            config.w3.grad.zero_()
                            config.w4.grad.zero_()

                            for name, param in self.net_avg.named_parameters():
                                if param.grad is not None:
                                    if param.grad.grad_fn is not None:
                                        param.grad.detach_()
                                    else:
                                        param.grad.requires_grad_(False)
                                    param.grad.zero_()

                        #dummy = input()
                        del out, inputs, targets, clean_targets
                        torch.cuda.empty_cache()




                #########################################################################################################
                #########################################################################################################
                #########################################################################################################

                if flr==1:
                    for inputs,labels in self.poisoned_train_loader_1:
                        inputs, targets = inputs.cuda(), labels.cuda()
                        out = self.net(inputs)
                        fea = out.view(out.size(0), -1)

                    features = fea
                    if(self.dataset == "triggerpatch"):
                        np.save("feature_500_trigger_car",features.cpu().detach().numpy())
                    if(self.dataset == "cifar10"):
                        np.save("feature_500_southwest",features.cpu().detach().numpy())

                if(self.dataset == "triggerpatch"):
                    fea = np.load('feature_500_trigger_car.npy')
                if(self.dataset == "cifar10"):
                    fea = np.load('feature_500_southwest.npy')
                #fea = fea[394:404]
                fea = torch.from_numpy(fea).cuda()

                print(fea.shape) #should be no. of images, each of 512 dimension
                ###############################################

                net_multiply1 = torch.matmul(config.w1.clone(), torch.t(fea).clone())
                relu = nn.ReLU()
                relu_net_multiply1 = relu(net_multiply1)

                net_multiply2 = torch.matmul(config.w2.clone(), relu_net_multiply1.clone())
                net_multiply2 = torch.t(net_multiply2)
                '''
                logsoftmax = nn.LogSoftmax(dim=1)
                softmax_net_multiply2 = logsoftmax(net_multiply2)
                '''
                softmax_net_multiply2 = torch.softmax(net_multiply2 , dim=1)

                #########################################################################
                y_one_hot = torch.zeros((500, 10), requires_grad=True).cuda()
                y_one_hot[torch.arange(500), self.poisoned_trainset.targets] = 1
                #y_one_hot = torch.zeros((10, 10), requires_grad=True).cuda()
                #y_one_hot[torch.arange(10), self.poisoned_trainset.targets[394:404]] = 1
                #########################################################################

                #print("one hot shape sould be (500,10)",y_one_hot.shape)

                concat_pred_given = torch.cat((softmax_net_multiply2, y_one_hot), 1)

                net_multiply3 = torch.matmul(config.w3.clone(), torch.t(concat_pred_given).clone())
                relu_net_multiply3 = relu(net_multiply3)

                net_multiply4 = torch.matmul(config.w4.clone(), relu_net_multiply3.clone())

                maxim = torch.max(net_multiply4)
                minim = torch.min(net_multiply4)

                if maxim!=0 or minim!=0:
                    gamma = torch.div(torch.sub(net_multiply4,minim) , torch.sub(maxim,minim))
                else:
                    gamma = torch.div(torch.sub(net_multiply4,minim) , torch.sub(maxim,minim)+0.000001)

                print("gamma = ",gamma)
                print(gamma.shape)
                gamma = gamma.squeeze(0)
                print(gamma.shape)
                ########################################################################################
                ########################################################################################

                gamma1 = gamma
                gamma1 = gamma1.cpu().detach().numpy()
                gamma_values.append(gamma1)

                ###################################################
                beta = 0.2
                ###################################################


                '''
                gamma_row = gamma.unsqueeze(0)
                gamma_col = gamma.unsqueeze(1)
                m = gamma.shape[0]
                _z = cp.Variable((1,m))
                _gamma_row = cp.Parameter((1,m))
                _gamma_col = cp.Parameter((m,1))

                objective = cp.Maximize(cp.sum((_z.T @ _gamma_row) - ((1 - _z) @ _gamma_col)))
                constraints = [0 <= _z, _z <= 1, cp.sum(_z)<=(beta * m)]
                prob = cp.Problem(objective, constraints)
                layer = CvxpyLayer(prob, parameters=[_gamma_row,_gamma_col], variables=[_z])
                z = layer(gamma_row,gamma_col)
                '''

                '''
                gamma_diff = gamma.unsqueeze(0)-gamma.unsqueeze(1)
                m = gamma.shape[0]
                _z = cp.Variable((1,m))
                _gamma_diff = cp.Parameter((m, m))

                objective = cp.Maximize(cp.sum(_z @ _gamma_diff))
                constraints = [0 <= _z, _z <= 1, cp.sum(_z)<=(beta * m)]
                prob = cp.Problem(objective, constraints)
                layer = CvxpyLayer(prob, parameters=[_gamma_diff], variables=[_z])
                z = layer(gamma_diff, solver_args={'solve_method':'ECOS'})
                '''

                '''
                print("z value = ", z)
                z= torch.stack(z)
                z= z.squeeze(0)
                z= z.squeeze(0)
                #print(z)
                '''

                dec_gamma_indices = torch.topk(gamma, gamma.shape[0])
                p_c_indices = dec_gamma_indices[1]
                poi_cle_indices = p_c_indices.cpu().detach().numpy()
                print("\nTop total gamma indices = ",poi_cle_indices)
                poison_clean_indices.append(poi_cle_indices)
                ##########################################################
                beta_int = int(beta * gamma.shape[0])
                poison_indices = poi_cle_indices[:beta_int]
                print("\npoison indices got from top gamma = ",poison_indices)

                red_in_top_beta=0
                green_in_top_beta =0

                for l in poison_indices:
                    if l in range(400,500):
                        red_in_top_beta = red_in_top_beta + 1
                    else:
                        green_in_top_beta = green_in_top_beta + 1

                print("\nNumber of actual poison points in top beta = ",red_in_top_beta)
                print("Number of actual clean points in top beta = ",green_in_top_beta)

                red_in_rest = 0
                green_in_rest = 0

                rest = list(set(poi_cle_indices) - set(poison_indices))

                for o in rest:
                    if o in range(400,500):
                        red_in_rest = red_in_rest + 1
                    else:
                        green_in_rest = green_in_rest + 1

                print("\nNumber of actual poison points in rest = ",red_in_rest)
                print("Number of actual clean points in rest = ",green_in_rest)

                red_top.append(red_in_top_beta)
                green_top.append(green_in_top_beta)
                red_rest.append(red_in_rest)
                green_rest.append(green_in_rest)

                ########################################################################################

                poison_indices = torch.tensor(poison_indices).cuda()

                class MyCustomDataset(Dataset):
                    #def __init__(self, trainX, trainy, height, width, poison_indices, z, transforms=None):
                    def __init__(self, trainX, trainy, height, width, poison_indices, gamma, transforms=None):
                        self.data = trainX
                        self.labels = trainy
                        self.height = height
                        self.width = width
                        self.poison_indices = poison_indices
                        #self.z = z
                        self.gamma = gamma
                        self.transforms = transforms

                    def __getitem__(self, index):
                        single_image_label = self.labels[index]
                        img_as_np = np.asarray(self.data[index])
                        img_as_img = Image.fromarray(img_as_np)
                        if self.transforms is not None:
                            img_as_tensor = self.transforms(img_as_img)
                        if index in self.poison_indices:
                            flag = 1
                        else:
                            flag = 0
                        #single_image_z = self.z[index]
                        single_image_gamma = self.gamma[index]

                        #return (img_as_tensor, single_image_label, flag, single_image_z)
                        return (img_as_tensor, single_image_label, flag, single_image_gamma)

                    def __len__(self):
                        return len(self.data) 

                transform_train = transforms.Compose([
                    transforms.RandomCrop(32, padding=4),
                    transforms.RandomHorizontalFlip(),
                    transforms.ToTensor(),
                    transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
                ])

                ###################################################################################################################
                poison_flag = MyCustomDataset(self.poisoned_trainset.data, self.poisoned_trainset.targets, 32, 32, poison_indices, gamma, transform_train)
                #poison_flag = MyCustomDataset(self.poisoned_trainset.data, self.poisoned_trainset.targets, 32, 32, poison_indices, z, transform_train)
                #poison_flag = MyCustomDataset(self.poisoned_trainset.data[394:404], self.poisoned_trainset.targets[394:404], 32, 32, poison_indices, z, transform_train)
                ###################################################################################################################
                poisoned_train_loader_flag = torch.utils.data.DataLoader(dataset = poison_flag, batch_size=32, shuffle=True)


                del poison_indices
                torch.cuda.empty_cache()





            if self.defense_technique == "no-defense":
                pass
            elif self.defense_technique == "norm-clipping":
                for net_idx, net in enumerate(net_list):
                    self._defender.exec(client_model=net, global_model=self.net_avg)
            elif self.defense_technique == "norm-clipping-adaptive":
                # we will need to adapt the norm diff first before the norm diff clipping
                logger.info("#### Let's Look at the Nom Diff Collector : {} ....; Mean: {}".format(norm_diff_collector, 
                    np.mean(norm_diff_collector)))
                self._defender.norm_bound = np.mean(norm_diff_collector)
                for net_idx, net in enumerate(net_list):
                    self._defender.exec(client_model=net, global_model=self.net_avg)
            elif self.defense_technique == "weak-dp":
                # this guy is just going to clip norm. No noise added here
                for net_idx, net in enumerate(net_list):
                    self._defender.exec(client_model=net,
                                        global_model=self.net_avg,)
            ###########################################################################
            elif self.defense_technique == "data-defense":
                net_freq = self._defender.exec(client_models=net_list, 
                                                        global_model=self.net_avg,
                                                        num_dps=[self.num_dps_poisoned_dataset]+num_data_points,
                                                        net_portion=net_freq,
                                                        poisoned_train_loader_flag = poisoned_train_loader_flag,
                                                        device=self.device)
            ###########################################################################
            elif self.defense_technique == "krum":
                net_list, net_freq = self._defender.exec(client_models=net_list,
                                                        num_dps=[self.num_dps_poisoned_dataset]+num_data_points,
                                                        g_user_indices=g_user_indices,
                                                        device=self.device)
            elif self.defense_technique == "multi-krum":
                net_list, net_freq = self._defender.exec(client_models=net_list,
                                                        num_dps=[self.num_dps_poisoned_dataset]+num_data_points,
                                                        g_user_indices=g_user_indices,
                                                        device=self.device)
            elif self.defense_technique == "rfa":
                net_list, net_freq = self._defender.exec(client_models=net_list,
                                                        net_freq=net_freq,
                                                        maxiter=500,
                                                        eps=1e-5,
                                                        ftol=1e-7,
                                                        device=self.device)
            else:
                NotImplementedError("Unsupported defense method !")

            # after local training periods
            ############################################################################################################################################

            # net_avg is modified by average (fed_avg_aggregator)
            # net list has the updated (10 net) trained on 10 client's sampled images
            # net freq of len 10 is the ratio of # client's images to the total number of images in that round

            if self.defense_technique != "data-defense":
                self.net_avg = fed_avg_aggregator(net_list, net_freq, device=self.device, model=self.model)


            if self.defense_technique == "data-defense":

                paramNettest = {} #dictionary of parameters for Net_test
                for name, param in net_list[0].named_parameters():
                    paramNettest[name] = torch.from_numpy(np.zeros(param.shape, dtype='float32')).cuda()

                #filling parameters for Net_test with Net1's parameters
                for i in range(self.part_nets_per_round):
                    for name, param in net_list[i].named_parameters():
                        paramNettest[name] += net_freq[i]*param

                #Initiating Net_test class with filled parameters

                new_model = vgg9.Net_test(paramNettest)
                new_model = new_model.cuda()

                #### UPDATING THE GLOBAL MODEL'S PARAMETERS ########
                self.net_avg.load_state_dict(paramNettest)

                ########################################################################################################################################

                def poison_loss(output,target):
                    output = output - torch.max(output)
                    X_exp_one = torch.exp(output[target])
                    X_exp_all = torch.exp(output)
                    partition = X_exp_all.sum(0, keepdim=True)
                    softmax = X_exp_one / partition
                    if softmax==1.0:
                        l_ap = -1.0 * torch.log(1.0000001 - softmax)
                    else:
                        l_ap = -1.0 * torch.log(1.0 - softmax)
                    return l_ap

                def clean_loss(output,target):
                    output = output - torch.max(output)
                    X_exp_one = torch.exp(output[target])
                    X_exp_all = torch.exp(output)
                    partition = X_exp_all.sum(0, keepdim=True)
                    softmax = X_exp_one / partition
                    if softmax==0.0:
                        l_ac = -1.0 * torch.log(0.0000001 + softmax)
                    else:
                        l_ac = -1.0 * torch.log(softmax)
                    return l_ac

                new_model.eval()


                for data in poisoned_train_loader_flag:

                    #inputs, targets, flag, z = data
                    #inputs, targets, flag, z = inputs.cuda(), targets.cuda(), flag.cuda(), z.cuda()

                    inputs, targets, flag, gamma = data
                    inputs, targets, flag, gamma = inputs.cuda(), targets.cuda(), flag.cuda(), gamma.cuda()

                    outputs = new_model(inputs)
                    loss_ap = torch.tensor([0.],requires_grad=True).cuda()
                    loss_ac = torch.tensor([0.],requires_grad=True).cuda()
                    loss = torch.tensor([0.],requires_grad=True).cuda()
                    sum_value_loss = torch.tensor([0.],requires_grad=True).cuda()
                    value_fn = torch.tensor([0.],requires_grad=True).cuda()
                    count_ap = 0.0
                    count_ac = 0.0

                    for i in range(inputs.shape[0]):

                        if flag[i] == 1:
                            l_ap = poison_loss(outputs[i],targets[i])
                            loss_ap = loss_ap + l_ap
                            count_ap = count_ap+1
                            Value_l_ap = l_ap
                            Value_l_ac = clean_loss(outputs[i],targets[i])
                            #sum_value_loss += z[i]*Value_l_ap + (1-z[i])*Value_l_ac
                            #sum_value_loss += z[i] * (Value_l_ap - Value_l_ac)
                            sum_value_loss += gamma[i] * (Value_l_ap - Value_l_ac)

                        else:
                            l_ac = clean_loss(outputs[i],targets[i])
                            loss_ac = loss_ac + l_ac
                            count_ac = count_ac+1
                            Value_l_ac = l_ac
                            Value_l_ap = poison_loss(outputs[i],targets[i])
                            #sum_value_loss += z[i]*Value_l_ap + (1-z[i])*Value_l_ac
                            #sum_value_loss += z[i] * (Value_l_ap - Value_l_ac)
                            sum_value_loss += gamma[i] * (Value_l_ap - Value_l_ac)


                    if count_ac !=0:
                        loss_ac = loss_ac/count_ac

                    if count_ac ==0:
                        loss_ac = loss_ac*0.0

                    if count_ap !=0:
                        loss_ap = loss_ap/count_ap

                    if count_ap ==0:
                        loss_ap = loss_ap*0.0

                    value_fn = sum_value_loss/inputs.shape[0]

                    #alpha = 0.75
                    #loss = (alpha*loss_ac) + ((1-alpha)*loss_ap)

                    loss = loss_ac + loss_ap

                    print("Loss_ac = " ,loss_ac)
                    print("Loss_ap = " ,loss_ap)
                    print("Loss = " ,loss)
                    print("Value function = " ,value_fn)

                    config.theta.retain_grad()
                    config.w1.retain_grad()
                    config.w2.retain_grad()
                    config.w3.retain_grad()
                    config.w4.retain_grad()

                    '''
                    # L2 REGULARISER
                    l2_lambda = 0.01
                    l2_reg = torch.tensor(0.,requires_grad=True).cuda()
                    for param in new_model.parameters():
                        l2_reg += torch.norm(param)
                    loss += l2_lambda * l2_reg
                    '''

                    loss.backward(retain_graph=True)
                    with torch.no_grad():

                        #print("Theta before gradient descent = ",config.theta)
                        #print("Gradient of loss wrt theta = ", config.theta.grad)
                        config.theta.sub_(config.theta.grad*0.01)
                        #print("Theta after gradient descent = ",config.theta)
                        config.theta.grad.zero_()


                        for i in range(self.part_nets_per_round):
                            for name, param in net_list[i].named_parameters():
                                if param.grad is not None:
                                    if param.grad.grad_fn is not None:
                                        param.grad.detach_()
                                    else:
                                        param.grad.requires_grad_(False)
                                    param.grad.zero_()

                        for name, param in self.net_avg.named_parameters():
                            if param.grad is not None:
                                if param.grad.grad_fn is not None:
                                    param.grad.detach_()
                                else:
                                    param.grad.requires_grad_(False)
                                param.grad.zero_()

                        for name,param in new_model.named_parameters():
                            if param.grad is not None:
                                if param.grad.grad_fn is not None:
                                    param.grad.detach_()
                                else:
                                    param.grad.requires_grad_(False)
                                param.grad.zero_()


                      

                    value_fn.backward(retain_graph=True)

                    with torch.no_grad():
                        '''
                        norm_W1_grad = torch.norm(config.w1.grad)
                        norm_W2_grad = torch.norm(config.w2.grad)
                        norm_W3_grad = torch.norm(config.w3.grad)
                        norm_W4_grad = torch.norm(config.w4.grad)
                        norm_grad_all = torch.tensor([norm_W1_grad, norm_W2_grad, norm_W3_grad, norm_W4_grad])
                        norm_psi = torch.norm(norm_grad_all)
                        psi_norm.append(norm_psi.item())
                        '''

                        #print("w1, w2 before gradient descent = ",config.w1, config.w2)
                        #print("Gradient of value fn wrt w1,w2 = ", config.w1.grad, config.w2.grad)

                        config.w1.sub_(config.w1.grad*self.gamma_lr)
                        config.w2.sub_(config.w2.grad*self.gamma_lr)
                        config.w3.sub_(config.w3.grad*self.gamma_lr)
                        config.w4.sub_(config.w4.grad*self.gamma_lr)
                        '''
                        config.w1.sub_(config.w1.grad*0.00001)
                        config.w2.sub_(config.w2.grad*0.00001)
                        config.w3.sub_(config.w3.grad*0.00001)
                        config.w4.sub_(config.w4.grad*0.00001)
                        '''

                        #print("w1, w2 after gradient descent = ",config.w1, config.w2)
                        config.w1.grad.zero_()
                        config.w2.grad.zero_()
                        config.w3.grad.zero_()
                        config.w4.grad.zero_()

                        for i in range(self.part_nets_per_round):
                            for name, param in net_list[i].named_parameters():
                                if param.grad is not None:
                                    if param.grad.grad_fn is not None:
                                        param.grad.detach_()
                                    else:
                                        param.grad.requires_grad_(False)
                                    param.grad.zero_()

                        for name, param in self.net_avg.named_parameters():
                            if param.grad is not None:
                                if param.grad.grad_fn is not None:
                                    param.grad.detach_()
                                else:
                                    param.grad.requires_grad_(False)
                                param.grad.zero_()

                        for name,param in new_model.named_parameters():
                            if param.grad is not None:
                                if param.grad.grad_fn is not None:
                                    param.grad.detach_()
                                else:
                                    param.grad.requires_grad_(False)
                                param.grad.zero_()

                    loss1 = loss.item()
                    self.loss_total.append(loss1)

                    loss2 = loss_ac.item()
                    self.loss_clean.append(loss2)

                    loss3 = loss_ap.item()
                    self.loss_poison.append(loss3)

                    value1 = value_fn.item()
                    self.value_total.append(value1)

                    #del outputs, loss, loss_ac, loss_ap,value_fn, inputs, targets, flag, z, sum_value_loss
                    del outputs, loss, loss_ac, loss_ap,value_fn, inputs, targets, flag, gamma, sum_value_loss
                    torch.cuda.empty_cache()

                paramNettest.clear()
                del new_model
            ######################################################################################################
            ######################################################################################################
            
            if self.defense_technique == "weak-dp":
                # add noise to self.net_avg
                noise_adder = AddNoise(stddev=self.stddev)
                noise_adder.exec(client_model=self.net_avg,
                                                device=self.device)

            v = torch.nn.utils.parameters_to_vector(self.net_avg.parameters())
            logger.info("############ Averaged Model : Norm {}".format(torch.norm(v)))
            
            # norm difference between the original global model and the average global model
            calc_norm_diff(gs_model=self.net_avg, vanilla_model=self.vanilla_model, epoch=0, fl_round=flr, mode="avg")
            
            logger.info("Measuring the accuracy of the averaged global model, FL round: {} ...".format(flr))
            
            # test on 10000 test images of cifar10
            # overall_acc means on 10000 test images, raw_acc means on 1000 truck images of cifar10 test set 
            overall_acc, raw_acc = test(self.net_avg, self.device, self.vanilla_emnist_test_loader, test_batch_size=self.test_batch_size, criterion=self.criterion, mode="raw-task_acc", dataset=self.dataset, poison_type=self.poison_type)
            
            
            # test on 196 southwest test images
            # backdoor_acc means test acc on 196 southwest test images (labeled as truck)
            backdoor_acc, _ = test(self.net_avg, self.device, self.targetted_task_test_loader, test_batch_size=self.test_batch_size, criterion=self.criterion, mode="targetted-task", dataset=self.dataset, poison_type=self.poison_type)
 
            # fl_iter_list is list of 0,1,2,...,500
            fl_iter_list.append(flr)
            # main task acc on 10000 test images of cifar10
            main_task_acc.append(overall_acc)
            # raw_task_acc on 1000 truck images of cifar10 test set
            raw_task_acc.append(raw_acc)
            # backdoor_acc on 196 southwest test images (labeled as truck)
            backdoor_task_acc.append(backdoor_acc)

            ###############################################################
            if self.defense_technique == "data-defense":
                freq_clients = net_freq.tolist()
                freq_list.append(freq_clients)


            if self.defense_technique == "data-defense":
                df2 = pd.DataFrame({'freq_list': freq_list}) 
                freq_filename = "freq_assigned.csv"
                df2.to_csv(freq_filename, index=False)
                logger.info("Wrote weights results to: {}".format(freq_filename))
        ###################################################################


            df = pd.DataFrame({'fl_iter': fl_iter_list, 
                            'main_task_acc': main_task_acc, 
                            'backdoor_acc': backdoor_task_acc, 
                            'raw_task_acc':raw_task_acc, 
                            'adv_norm_diff': adv_norm_diff_list, 
                            'wg_norm': wg_norm_list
                            })
       
            if self.poison_type == 'ardis':
                # add a row showing initial accuracies
                df1 = pd.DataFrame({'fl_iter': [0], 'main_task_acc': [88], 'backdoor_acc': [11], 'raw_task_acc': [0], 'adv_norm_diff': [0], 'wg_norm': [0]})
                df = pd.concat([df1, df])

            results_filename = get_results_filename(self.poison_type, self.attack_method, self.model_replacement, self.project_frequency, self.defense_technique, self.norm_bound, self.prox_attack, False, self.model)

            df.to_csv(results_filename, index=False)
            logger.info("Wrote accuracy results to: {}".format(results_filename))


            if self.defense_technique == "data-defense":

                folder1 = f"./loss_learning"
                np.savez(f'{folder1}', loss_total = self.loss_total, loss_clean = self.loss_clean, loss_poison = self.loss_poison)

                np.save("value_total",self.value_total)

                np.save("gamma_values",gamma_values)

                np.save("poison_clean_indices",poison_clean_indices)

                np.save("ce_loss",ce_loss)

                np.save("red_top",red_top)
                np.save("green_top",green_top)
                np.save("red_rest",red_rest)
                np.save("green_rest",green_rest)

                np.save("psi_norm",psi_norm)

                if flr==1:
                    np.save("gamma_diff1",gamma_diff1)


            np.save("class_main_task_acc",class_main_task_acc)

            #folder6 = f"./class_backdoor_acc"
            #np.savez(f'{folder6}', class_backdoor_acc = class_backdoor_acc)

            # save model net_avg
            # torch.save(self.net_avg.state_dict(), "./checkpoint/emnist_lenet_10epoch.pt")
        
############################################################################################
############################################################################################
############################################################################################
############################################################################################
############################################################################################
############################################################################################
############################################################################################
############################################################################################
############################################################################################
############################################################################################
############################################################################################
############################################################################################
############################################################################################
############################################################################################
############################################################################################
############################################################################################
############################################################################################
############################################################################################

class FixedPoolFederatedLearningTrainer(FederatedLearningTrainer):
    def __init__(self, arguments=None, *args, **kwargs):

        #self.poisoned_emnist_dataset = arguments['poisoned_emnist_dataset']
        self.vanilla_model = arguments['vanilla_model']
        self.net_avg = arguments['net_avg']
        self.net = arguments['net']
        self.net_dataidx_map = arguments['net_dataidx_map']
        self.num_nets = arguments['num_nets']
        self.part_nets_per_round = arguments['part_nets_per_round']
        self.fl_round = arguments['fl_round']
        self.local_training_period = arguments['local_training_period']
        self.adversarial_local_training_period = arguments['adversarial_local_training_period']
        self.args_lr = arguments['args_lr']
        self.args_gamma = arguments['args_gamma']
        self.attacker_pool_size = arguments['attacker_pool_size']
        self.poisoned_emnist_train_loader = arguments['poisoned_emnist_train_loader']
        self.clean_train_loader = arguments['clean_train_loader']
        self.mislabeled_train_loader = arguments['mislabeled_train_loader']
        self.vanilla_emnist_test_loader = arguments['vanilla_emnist_test_loader']
        self.targetted_task_test_loader = arguments['targetted_task_test_loader']
        self.poisoned_trainset = arguments['poisoned_trainset']
        self.poisoned_train_loader_1 = arguments['poisoned_train_loader_1']
        self.batch_size = arguments['batch_size']
        self.test_batch_size = arguments['test_batch_size']
        self.log_interval = arguments['log_interval']
        self.device = arguments['device']
        self.dataset = arguments["dataset"]
        self.model = arguments["model"]
        self.num_dps_poisoned_dataset = arguments['num_dps_poisoned_dataset']
        self.defense_technique = arguments["defense_technique"]
        self.norm_bound = arguments["norm_bound"]
        self.attack_method = arguments["attack_method"]
        self.criterion = nn.CrossEntropyLoss()
        self.eps = arguments['eps']
        self.dataset = arguments["dataset"]
        self.poison_type = arguments['poison_type']
        self.model_replacement = arguments['model_replacement']
        self.project_frequency = arguments['project_frequency']
        self.adv_lr = arguments['adv_lr']
        self.prox_attack = arguments['prox_attack']
        self.attack_case = arguments['attack_case']
        self.stddev = arguments['stddev']
        self.loss_clean = arguments['loss_clean']
        self.loss_poison = arguments['loss_poison']
        self.loss_total = arguments['loss_total']
        self.value_total = arguments['value_total']
        self.gamma_lr = arguments['gamma_lr']

        logger.info("Posion type! {}".format(self.poison_type))

        if self.attack_method == "pgd":
            self.pgd_attack = True
        else:
            self.pgd_attack = False

        if self.poison_type == 'ardis':
            self.ardis_dataset = datasets.get_ardis_dataset()
            # exclude first 66 points because they are part of the adversary
            if self.attack_case == 'normal-case':
                self.ardis_dataset.data = self.ardis_dataset.data[66:]
            elif self.attack_case == 'almost-edge-case':
                self.ardis_dataset.data = self.ardis_dataset.data[66:132]
        elif self.poison_type == 'southwest':
            self.ardis_dataset = datasets.get_southwest_dataset(attack_case=self.attack_case)
        else:
            self.ardis_dataset=None


        if arguments["defense_technique"] == "no-defense":
            self._defender = None
        elif arguments["defense_technique"] == "norm-clipping" or arguments["defense_technique"] == "norm-clipping-adaptive":
            self._defender = WeightDiffClippingDefense(norm_bound=arguments['norm_bound'])
        elif arguments["defense_technique"] == "weak-dp":
            # doesn't really add noise. just clips
            self._defender = WeightDiffClippingDefense(norm_bound=arguments['norm_bound'])
        ############################################################
        elif arguments["defense_technique"] == "data-defense":
            self._defender = LearningDefense()
        ############################################################
        elif arguments["defense_technique"] == "krum":
            self._defender = Krum(mode='krum', num_workers=self.part_nets_per_round, num_adv=1)
        elif arguments["defense_technique"] == "multi-krum":
            self._defender = Krum(mode='multi-krum', num_workers=self.part_nets_per_round, num_adv=1)
        elif arguments["defense_technique"] == "rfa":
            self._defender = RFA()
        else:
            NotImplementedError("Unsupported defense method !")

        self.__attacker_pool = np.random.choice(self.num_nets, self.attacker_pool_size, replace=False)

    def run(self):

        main_task_acc = []
        raw_task_acc = []
        backdoor_task_acc = []
        fl_iter_list = []
        adv_norm_diff_list = []
        wg_norm_list = []
        freq_list = []
        poison_clean_indices = []
        gamma_values = []
        gamma_diff1 = []
        ce_loss = []
        red_top = []
        green_top = []
        red_rest = []
        green_rest = []
        psi_norm = []

        # let's conduct multi-round training
        for flr in range(1, self.fl_round+1):
            g_user_indices = []
            # randomly select participating clients
            # in this current version, we sample `part_nets_per_round` per FL round since we assume attacker will always participates
            selected_node_indices = np.random.choice(self.num_nets, size=self.part_nets_per_round, replace=False)

            selected_attackers = [idx for idx in selected_node_indices if idx in self.__attacker_pool]
            selected_honest_users = [idx for idx in selected_node_indices if idx not in self.__attacker_pool]
            logger.info("Selected Attackers in FL iteration-{}: {}".format(flr, selected_attackers))

            num_data_points = []
            for sni in selected_node_indices:
                if sni in selected_attackers:
                    num_data_points.append(self.num_dps_poisoned_dataset)
                else:
                    num_data_points.append(len(self.net_dataidx_map[sni]))

            total_num_dps_per_round = sum(num_data_points)
            net_freq = [num_data_points[i]/total_num_dps_per_round for i in range(self.part_nets_per_round)]
            logger.info("Net freq: {}, FL round: {} with adversary".format(net_freq, flr)) 

            # we need to reconstruct the net list at the beginning
            net_list = [copy.deepcopy(self.net_avg) for _ in range(self.part_nets_per_round)]
            logger.info("################## Starting fl round: {}".format(flr))
            model_original = list(self.net_avg.parameters())
            # super hacky but I'm doing this for the prox-attack
            wg_clone = copy.deepcopy(self.net_avg)
            wg_hat = None
            v0 = torch.nn.utils.parameters_to_vector(model_original)
            wg_norm_list.append(torch.norm(v0).item())


            #     # start the FL process
            for net_idx, global_user_idx in enumerate(selected_node_indices):
                net  = net_list[net_idx]
                if global_user_idx in selected_attackers:
                    pass
                else:
                    dataidxs = self.net_dataidx_map[global_user_idx]
                    #train_dl_local, _ = get_dataloader(self.dataset, './data', self.batch_size, 
                    #                                self.test_batch_size, dataidxs) # also get the data loader

                    # add p-percent edge-case attack here:
                    if self.attack_case == "edge-case":
                        train_dl_local, _ = get_dataloader(self.dataset, './data', self.batch_size, 
                                                       self.test_batch_size, dataidxs) # also get the data loader
                    elif self.attack_case in ("normal-case", "almost-edge-case"):
                        train_dl_local, _ = get_dataloader_normal_case(self.dataset, './data', self.batch_size, 
                                                            self.test_batch_size, dataidxs, user_id=global_user_idx,
                                                            num_total_users=self.num_nets,
                                                            poison_type=self.poison_type,
                                                            ardis_dataset=self.ardis_dataset,
                                                            attack_case=self.attack_case) # also get the data loader
                    else:
                        NotImplementedError("Unsupported attack case ...")

                g_user_indices.append(global_user_idx)

                logger.info("@@@@@@@@ Working on client (global-index): {}, which {}-th user in the current round".format(global_user_idx, net_idx))

                #criterion = nn.CrossEntropyLoss()
                #optimizer = optim.SGD(net.parameters(), lr=self.args_lr*self.args_gamma**(flr-1), momentum=0.9, weight_decay=1e-4) # epoch, net, train_loader, optimizer, criterion
                #for param_group in optimizer.param_groups:
                #    logger.info("Effective lr in FL round: {} is {}".format(flr, param_group['lr']))                

                criterion = nn.CrossEntropyLoss()
                optimizer = optim.SGD(net.parameters(), lr=self.args_lr*self.args_gamma**(flr-1), momentum=0.9, weight_decay=1e-4) # epoch, net, train_loader, optimizer, criterion
                adv_optimizer = optim.SGD(net.parameters(), lr=self.adv_lr*self.args_gamma**(flr-1), momentum=0.9, weight_decay=1e-4) # looks like adversary needs same lr to hide with others
                prox_optimizer = optim.SGD(wg_clone.parameters(), lr=self.args_lr*self.args_gamma**(flr-1), momentum=0.9, weight_decay=1e-4)
                for param_group in optimizer.param_groups:
                    logger.info("Effective lr in FL round: {} is {}".format(flr, param_group['lr']))


                current_adv_norm_diff_list = []
                if global_user_idx in selected_attackers:
                    # for e in range(1, self.adversarial_local_training_period+1):
                    #    # we always assume net index 0 is adversary
                    #    train(net, self.device, self.poisoned_emnist_train_loader, optimizer, e, log_interval=self.log_interval, criterion=self.criterion)

                    # logger.info("=====> Measuring the model performance of the poisoned model after attack ....")
                    # test(net, self.device, self.vanilla_emnist_test_loader, test_batch_size=self.test_batch_size, criterion=self.criterion, mode="raw-task", dataset=self.dataset, poison_type=self.poison_type)
                    # test(net, self.device, self.targetted_task_test_loader, test_batch_size=self.test_batch_size, criterion=self.criterion, mode="targetted-task", dataset=self.dataset, poison_type=self.poison_type)
                    # # at here we can check the distance between w_bad and w_g i.e. `\|w_bad - w_g\|_2`
                    # calc_norm_diff(gs_model=net, vanilla_model=self.net_avg, epoch=e, fl_round=flr, mode="bad")

                    if self.prox_attack:
                        # estimate w_hat
                        for inner_epoch in range(1, self.local_training_period+1):
                            estimate_wg(wg_clone, self.device, self.clean_train_loader, prox_optimizer, inner_epoch, log_interval=self.log_interval, criterion=self.criterion)
                        wg_hat = wg_clone

                    for e in range(1, self.adversarial_local_training_period+1):
                       # we always assume net index 0 is adversary
                        if self.defense_technique in ('krum', 'multi-krum'):
                            train(net, self.device, self.poisoned_emnist_train_loader, optimizer, e, log_interval=self.log_interval, criterion=self.criterion,
                                    pgd_attack=self.pgd_attack, eps=self.eps*self.args_gamma**(flr-1), model_original=model_original, project_frequency=self.project_frequency, adv_optimizer=adv_optimizer,
                                    prox_attack=self.prox_attack, wg_hat=wg_hat)
                        else:
                            train(net, self.device, self.poisoned_emnist_train_loader, optimizer, e, log_interval=self.log_interval, criterion=self.criterion,
                                    pgd_attack=self.pgd_attack, eps=self.eps, model_original=model_original, project_frequency=self.project_frequency, adv_optimizer=adv_optimizer,
                                    prox_attack=self.prox_attack, wg_hat=wg_hat)

                    test(net, self.device, self.vanilla_emnist_test_loader, test_batch_size=self.test_batch_size, criterion=self.criterion, mode="raw-task", dataset=self.dataset, poison_type=self.poison_type)
                    test(net, self.device, self.targetted_task_test_loader, test_batch_size=self.test_batch_size, criterion=self.criterion, mode="targetted-task", dataset=self.dataset, poison_type=self.poison_type)

                    # if model_replacement scale models
                    if self.model_replacement:
                        v = torch.nn.utils.parameters_to_vector(net.parameters())
                        logger.info("Attacker before scaling : Norm = {}".format(torch.norm(v)))
                        # adv_norm_diff = calc_norm_diff(gs_model=net, vanilla_model=self.net_avg, epoch=e, fl_round=flr, mode="bad")
                        # logger.info("||w_bad - w_avg|| before scaling = {}".format(adv_norm_diff))

                        for idx, param in enumerate(net.parameters()):
                            param.data = (param.data - model_original[idx])*(total_num_dps_per_round/self.num_dps_poisoned_dataset) + model_original[idx]
                        v = torch.nn.utils.parameters_to_vector(net.parameters())
                        logger.info("Attacker after scaling : Norm = {}".format(torch.norm(v)))

                    # at here we can check the distance between w_bad and w_g i.e. `\|w_bad - w_g\|_2`
                    # we can print the norm diff out for debugging
                    adv_norm_diff = calc_norm_diff(gs_model=net, vanilla_model=self.net_avg, epoch=e, fl_round=flr, mode="bad")
                    current_adv_norm_diff_list.append(adv_norm_diff)

                    if self.defense_technique == "norm-clipping-adaptive":
                        # experimental
                        norm_diff_collector.append(adv_norm_diff)
                else:
                    # for e in range(1, self.local_training_period+1):
                    #    train(net, self.device, train_dl_local, optimizer, e, log_interval=self.log_interval, criterion=self.criterion)                
                    # # at here we can check the distance between w_normal and w_g i.e. `\|w_bad - w_g\|_2`
                    # calc_norm_diff(gs_model=net, vanilla_model=self.net_avg, epoch=e, fl_round=flr, mode="normal")

                    for e in range(1, self.local_training_period+1):
                       train(net, self.device, train_dl_local, optimizer, e, log_interval=self.log_interval, criterion=self.criterion)                
                       # at here we can check the distance between w_normal and w_g i.e. `\|w_bad - w_g\|_2`
                    # we can print the norm diff out for debugging
                    honest_norm_diff = calc_norm_diff(gs_model=net, vanilla_model=self.net_avg, epoch=e, fl_round=flr, mode="normal")
                    
                    if self.defense_technique == "norm-clipping-adaptive":
                        # experimental
                        norm_diff_collector.append(honest_norm_diff)


            ##################################################################################################################
            if self.defense_technique == "learning-defense":

                if (flr % 300) == 1:
                    self.gamma_lr = self.gamma_lr*0.1
                    print("gamma_lr = ",self.gamma_lr)

                # Random 100 indices out of 400 clean images are called D_clean
                if flr==1:

                    D_clean = np.random.choice(400, 100, replace=False)
                    '''
                    D_correct = np.random.choice(400, 85, replace=False)
                    poison = np.arange(400,500)
                    D_wrong = np.random.choice(poison, 15, replace=False)
                    D_clean = np.append(D_correct, D_wrong)
                    '''

                    D_clean = np.sort(D_clean)

                    sampled_100_indices = D_clean

                    indices_500 = np.arange(500)
                    #indices_500 = np.arange(10)

                    sampled_400_indices = list(set(indices_500) - set(sampled_100_indices))
                    ########################################################################
                    print(sampled_100_indices)
                    print(sampled_400_indices)
                    ########################################################################

                    for epoch in range(100):

                        for inputs,labels in self.poisoned_train_loader_1:
                            inputs, targets = inputs.cuda(), labels.cuda()
                            out = self.net(inputs)
                            feature_500 = out.view(out.size(0), -1)

                        print(feature_500.shape) #should be no. of images, each of 512 dimension

                        feature_100_clean = feature_500[sampled_100_indices]
                        feature_400_not_known = feature_500[sampled_400_indices]

                        # ***************************************************************************************************************

                        #print("w1",config.w1)
                        #print("w2",config.w2)
                        #print("w3",config.w3)
                        #print("w4",config.w4)

                        net_100_multiply1 = torch.matmul(config.w1.clone(), torch.t(feature_100_clean).clone())
                        net_400_multiply1 = torch.matmul(config.w1.clone(), torch.t(feature_400_not_known).clone())

                        print("net_100_multiply1",net_100_multiply1)
                        print("net_400_multiply1",net_400_multiply1)

                        relu = nn.ReLU()
                        relu_net_100_multiply1 = relu(net_100_multiply1)
                        relu_net_400_multiply1 = relu(net_400_multiply1)

                        net_100_multiply2 = torch.matmul(config.w2.clone(), relu_net_100_multiply1.clone())
                        net_400_multiply2 = torch.matmul(config.w2.clone(), relu_net_400_multiply1.clone())

                        net_100_multiply2 = torch.t(net_100_multiply2)
                        net_400_multiply2 = torch.t(net_400_multiply2)

                        #print("net_100_multiply2 shape sould be (10,clean)",net_100_multiply2.shape)
                        '''
                        logsoftmax = nn.LogSoftmax(dim=1)
                        softmax_net_100_multiply2 = logsoftmax(net_100_multiply2)
                        softmax_net_400_multiply2 = logsoftmax(net_400_multiply2)
                        '''
                        softmax_net_100_multiply2 = torch.softmax(net_100_multiply2, dim=1)
                        softmax_net_400_multiply2 = torch.softmax(net_400_multiply2, dim=1)

                        print("net_100_multiply2",net_100_multiply2)
                        print("net_400_multiply2",net_400_multiply2)
                        print("softmax_net_100_multiply2",softmax_net_100_multiply2)
                        print("softmax_net_400_multiply2",softmax_net_400_multiply2)

                        # Check 2: Check the shape of output of softmax
                        #print("softmax shape sould be (clean,10)",softmax_net_100_multiply2.shape)

                        # Check 3: Find one hot vector of y labels of both datasets

                        #############################################################################################
                        #y_one_hot_100 = torch.zeros((2, 10), requires_grad=False).cuda()
                        #y_one_hot_400 = torch.zeros((8, 10), requires_grad=False).cuda()
                        #y_one_hot_100[torch.arange(2), self.poisoned_trainset.targets[394:396]] = 1
                        #y_one_hot_400[torch.arange(8), self.poisoned_trainset.targets[396:404]] = 1

                        y_one_hot_100 = torch.zeros((100, 10), requires_grad=False).cuda()
                        y_one_hot_400 = torch.zeros((400, 10), requires_grad=False).cuda()
                        y_one_hot_100[torch.arange(100), self.poisoned_trainset.targets[sampled_100_indices]] = 1
                        y_one_hot_400[torch.arange(400), self.poisoned_trainset.targets[sampled_400_indices]] = 1
                        #############################################################################################

                        print("y one hot 100 output", y_one_hot_100)
                        print("y one hot 400 output", y_one_hot_400)

                        ###########################################################################
                        #clean_targets = self.poisoned_trainset.targets[394:396]
                        clean_targets = self.poisoned_trainset.targets[sampled_100_indices]

                        clean_targets = torch.from_numpy(clean_targets).cuda()
                        clean_targets = clean_targets.long()
                        cross_loss = nn.CrossEntropyLoss()
                        ce_clean_loss = cross_loss(softmax_net_100_multiply2, clean_targets)
                        ce_loss.append(ce_clean_loss.item())
                        ###########################################################################

                        concat_pred_given_100 = torch.cat((softmax_net_100_multiply2, y_one_hot_100), 1)
                        concat_pred_given_400 = torch.cat((softmax_net_400_multiply2, y_one_hot_400), 1)

                        net_100_multiply3 = torch.matmul(config.w3.clone(), torch.t(concat_pred_given_100).clone())
                        net_400_multiply3 = torch.matmul(config.w3.clone(), torch.t(concat_pred_given_400).clone())

                        relu_net_100_multiply3 = relu(net_100_multiply3)
                        relu_net_400_multiply3 = relu(net_400_multiply3)

                        print("net_100_multiply3",net_100_multiply3)
                        print("net_400_multiply3",net_400_multiply3)
                        print("relu_net_100_multiply3",relu_net_100_multiply3)
                        print("relu_net_400_multiply3",relu_net_400_multiply3)

                        net_100_multiply4 = torch.matmul(config.w4.clone(), relu_net_100_multiply3.clone())
                        net_400_multiply4 = torch.matmul(config.w4.clone(), relu_net_400_multiply3.clone())

                        print("net_100_multiply4",net_100_multiply4)
                        print("net_400_multiply4",net_400_multiply4)

                        max_100 = torch.max(net_100_multiply4)
                        max_400 = torch.max(net_400_multiply4)
                        maxi = torch.max(max_100 , max_400)

                        min_100 = torch.min(net_100_multiply4)
                        min_400 = torch.min(net_400_multiply4)
                        mini = torch.min(min_100, min_400)

                        print("max_100",max_100)
                        print("max_400",max_400)
                        print("min_100",min_100)
                        print("min_400",min_400)

                        if maxi !=0 or mini!=0:
                            gamma_100 = torch.div(torch.sub(net_100_multiply4,mini) , torch.sub(maxi,mini))
                            gamma_400 = torch.div(torch.sub(net_400_multiply4,mini) , torch.sub(maxi,mini))
                        else:
                            gamma_100 = torch.div(torch.sub(net_100_multiply4,mini) , torch.sub(maxi,mini)+0.0000001)
                            gamma_400 = torch.div(torch.sub(net_400_multiply4,mini) , torch.sub(maxi,mini)+0.0000001)

                        print("maxi",maxi)
                        print("mini",mini)

                        print("gamma_100",gamma_100)
                        print("gamma_400",gamma_400)

                        gamma_100 = gamma_100.squeeze(0)
                        gamma_400 = gamma_400.squeeze(0)

                        print(gamma_100.shape , gamma_400.shape)

                        diff_gamma = gamma_100.unsqueeze(0)-gamma_400.unsqueeze(1)
                        print("diff_gamma",diff_gamma)

                        #####################################################
                        new_loss = diff_gamma.sum() + 1000*ce_clean_loss
                        new_loss.backward(retain_graph=True)
                        #####################################################

                        gamma_diff1.append(diff_gamma.sum().item())

                        with torch.no_grad():

                            for param in self.net.parameters():
                                param = param - 0.00001 * param.grad
                            for param in self.net.parameters():
                                param.grad.zero_()

                            #print("\n w1, w2, w3, w4 before gradient descent = ",config.w1, config.w2, config.w3, config.w4)
                            #print("\n Grad of gammadiff wrt w1, w2, w3, w4 = ","\n\n", config.w1.grad,"\n\n",config.w2.grad,"\n\n", config.w3.grad,"\n\n", config.w4.grad)
                            config.w1.sub_(config.w1.grad*0.00001)
                            config.w2.sub_(config.w2.grad*0.00001)
                            config.w3.sub_(config.w3.grad*0.00001)
                            config.w4.sub_(config.w4.grad*0.00001)
                            #print("\n w1, w2, w3, w4 after gradient descent = ",config.w1, config.w2, config.w3, config.w4)

                            config.w1.grad.zero_()
                            config.w2.grad.zero_()
                            config.w3.grad.zero_()
                            config.w4.grad.zero_()

                            for name, param in self.net_avg.named_parameters():
                                if param.grad is not None:
                                    if param.grad.grad_fn is not None:
                                        param.grad.detach_()
                                    else:
                                        param.grad.requires_grad_(False)
                                    param.grad.zero_()

                        #dummy = input()
                        del out, inputs, targets, clean_targets
                        torch.cuda.empty_cache()




                #########################################################################################################
                #########################################################################################################
                #########################################################################################################

                if flr==1:
                    for inputs,labels in self.poisoned_train_loader_1:
                        inputs, targets = inputs.cuda(), labels.cuda()
                        out = self.net(inputs)
                        fea = out.view(out.size(0), -1)

                    features = fea
                    if(self.dataset == "triggerpatch"):
                        np.save("feature_500_trigger_car",features.cpu().detach().numpy())
                    if(self.dataset == "cifar10"):
                        np.save("feature_500_southwest",features.cpu().detach().numpy())

                if(self.dataset == "triggerpatch"):
                    fea = np.load('feature_500_trigger_car.npy')
                if(self.dataset == "cifar10"):
                    fea = np.load('feature_500_southwest.npy')
                #fea = fea[394:404]
                fea = torch.from_numpy(fea).cuda()

                print(fea.shape) #should be no. of images, each of 512 dimension
                ###############################################

                net_multiply1 = torch.matmul(config.w1.clone(), torch.t(fea).clone())
                relu = nn.ReLU()
                relu_net_multiply1 = relu(net_multiply1)

                net_multiply2 = torch.matmul(config.w2.clone(), relu_net_multiply1.clone())
                net_multiply2 = torch.t(net_multiply2)
                '''
                logsoftmax = nn.LogSoftmax(dim=1)
                softmax_net_multiply2 = logsoftmax(net_multiply2)
                '''
                softmax_net_multiply2 = torch.softmax(net_multiply2 , dim=1)

                #########################################################################
                y_one_hot = torch.zeros((500, 10), requires_grad=True).cuda()
                y_one_hot[torch.arange(500), self.poisoned_trainset.targets] = 1
                #y_one_hot = torch.zeros((10, 10), requires_grad=True).cuda()
                #y_one_hot[torch.arange(10), self.poisoned_trainset.targets[394:404]] = 1
                #########################################################################

                #print("one hot shape sould be (500,10)",y_one_hot.shape)

                concat_pred_given = torch.cat((softmax_net_multiply2, y_one_hot), 1)

                net_multiply3 = torch.matmul(config.w3.clone(), torch.t(concat_pred_given).clone())
                relu_net_multiply3 = relu(net_multiply3)

                net_multiply4 = torch.matmul(config.w4.clone(), relu_net_multiply3.clone())

                maxim = torch.max(net_multiply4)
                minim = torch.min(net_multiply4)

                if maxim!=0 or minim!=0:
                    gamma = torch.div(torch.sub(net_multiply4,minim) , torch.sub(maxim,minim))
                else:
                    gamma = torch.div(torch.sub(net_multiply4,minim) , torch.sub(maxim,minim)+0.000001)

                print("gamma = ",gamma)
                print(gamma.shape)
                gamma = gamma.squeeze(0)
                print(gamma.shape)
                ########################################################################################
                ########################################################################################

                gamma1 = gamma
                gamma1 = gamma1.cpu().detach().numpy()
                gamma_values.append(gamma1)

                ###################################################
                beta = 0.2
                ###################################################


                '''
                gamma_row = gamma.unsqueeze(0)
                gamma_col = gamma.unsqueeze(1)
                m = gamma.shape[0]
                _z = cp.Variable((1,m))
                _gamma_row = cp.Parameter((1,m))
                _gamma_col = cp.Parameter((m,1))

                objective = cp.Maximize(cp.sum((_z.T @ _gamma_row) - ((1 - _z) @ _gamma_col)))
                constraints = [0 <= _z, _z <= 1, cp.sum(_z)<=(beta * m)]
                prob = cp.Problem(objective, constraints)
                layer = CvxpyLayer(prob, parameters=[_gamma_row,_gamma_col], variables=[_z])
                z = layer(gamma_row,gamma_col)
                '''

                '''
                gamma_diff = gamma.unsqueeze(0)-gamma.unsqueeze(1)
                m = gamma.shape[0]
                _z = cp.Variable((1,m))
                _gamma_diff = cp.Parameter((m, m))

                objective = cp.Maximize(cp.sum(_z @ _gamma_diff))
                constraints = [0 <= _z, _z <= 1, cp.sum(_z)<=(beta * m)]
                prob = cp.Problem(objective, constraints)
                layer = CvxpyLayer(prob, parameters=[_gamma_diff], variables=[_z])
                z = layer(gamma_diff, solver_args={'solve_method':'ECOS'})
                '''

                '''
                print("z value = ", z)
                z= torch.stack(z)
                z= z.squeeze(0)
                z= z.squeeze(0)
                #print(z)
                '''

                dec_gamma_indices = torch.topk(gamma, gamma.shape[0])
                p_c_indices = dec_gamma_indices[1]
                poi_cle_indices = p_c_indices.cpu().detach().numpy()
                print("\nTop total gamma indices = ",poi_cle_indices)
                poison_clean_indices.append(poi_cle_indices)
                ##########################################################
                beta_int = int(beta * gamma.shape[0])
                poison_indices = poi_cle_indices[:beta_int]
                print("\npoison indices got from top gamma = ",poison_indices)

                red_in_top_beta=0
                green_in_top_beta =0

                for l in poison_indices:
                    if l in range(400,500):
                        red_in_top_beta = red_in_top_beta + 1
                    else:
                        green_in_top_beta = green_in_top_beta + 1

                print("\nNumber of actual poison points in top beta = ",red_in_top_beta)
                print("Number of actual clean points in top beta = ",green_in_top_beta)

                red_in_rest = 0
                green_in_rest = 0

                rest = list(set(poi_cle_indices) - set(poison_indices))

                for o in rest:
                    if o in range(400,500):
                        red_in_rest = red_in_rest + 1
                    else:
                        green_in_rest = green_in_rest + 1

                print("\nNumber of actual poison points in rest = ",red_in_rest)
                print("Number of actual clean points in rest = ",green_in_rest)

                red_top.append(red_in_top_beta)
                green_top.append(green_in_top_beta)
                red_rest.append(red_in_rest)
                green_rest.append(green_in_rest)

                ########################################################################################

                poison_indices = torch.tensor(poison_indices).cuda()

                class MyCustomDataset(Dataset):
                    #def __init__(self, trainX, trainy, height, width, poison_indices, z, transforms=None):
                    def __init__(self, trainX, trainy, height, width, poison_indices, gamma, transforms=None):
                        self.data = trainX
                        self.labels = trainy
                        self.height = height
                        self.width = width
                        self.poison_indices = poison_indices
                        #self.z = z
                        self.gamma = gamma
                        self.transforms = transforms

                    def __getitem__(self, index):
                        single_image_label = self.labels[index]
                        img_as_np = np.asarray(self.data[index])
                        img_as_img = Image.fromarray(img_as_np)
                        if self.transforms is not None:
                            img_as_tensor = self.transforms(img_as_img)
                        if index in self.poison_indices:
                            flag = 1
                        else:
                            flag = 0
                        #single_image_z = self.z[index]
                        single_image_gamma = self.gamma[index]

                        #return (img_as_tensor, single_image_label, flag, single_image_z)
                        return (img_as_tensor, single_image_label, flag, single_image_gamma)

                    def __len__(self):
                        return len(self.data) 

                transform_train = transforms.Compose([
                    transforms.RandomCrop(32, padding=4),
                    transforms.RandomHorizontalFlip(),
                    transforms.ToTensor(),
                    transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
                ])

                ###################################################################################################################
                poison_flag = MyCustomDataset(self.poisoned_trainset.data, self.poisoned_trainset.targets, 32, 32, poison_indices, gamma, transform_train)
                #poison_flag = MyCustomDataset(self.poisoned_trainset.data, self.poisoned_trainset.targets, 32, 32, poison_indices, z, transform_train)
                #poison_flag = MyCustomDataset(self.poisoned_trainset.data[394:404], self.poisoned_trainset.targets[394:404], 32, 32, poison_indices, z, transform_train)
                ###################################################################################################################
                poisoned_train_loader_flag = torch.utils.data.DataLoader(dataset = poison_flag, batch_size=32, shuffle=True)


                del poison_indices
                torch.cuda.empty_cache()




            ##################################################################################################################

            if self.defense_technique == "no-defense":
                pass
            elif self.defense_technique == "norm-clipping":
                for net_idx, net in enumerate(net_list):
                    self._defender.exec(client_model=net, global_model=self.net_avg)
            elif self.defense_technique == "norm-clipping-adaptive":
                # we will need to adapt the norm diff first before the norm diff clipping
                logger.info("#### Let's Look at the Nom Diff Collector : {} ....; Mean: {}".format(norm_diff_collector, 
                    np.mean(norm_diff_collector)))
                self._defender.norm_bound = np.mean(norm_diff_collector)
                for net_idx, net in enumerate(net_list):
                    self._defender.exec(client_model=net, global_model=self.net_avg)
            elif self.defense_technique == "weak-dp":
                # this guy is just going to clip norm. No noise added here
                for net_idx, net in enumerate(net_list):
                    self._defender.exec(client_model=net,
                                        global_model=self.net_avg,)
            ###########################################################################
            elif self.defense_technique == "data-defense":
                net_freq = self._defender.exec(client_models=net_list, 
                                                        global_model=self.net_avg,
                                                        num_dps=[self.num_dps_poisoned_dataset]+num_data_points,
                                                        net_portion=net_freq,
                                                        poisoned_train_loader_flag = poisoned_train_loader_flag,
                                                        device=self.device)
            ###########################################################################
            elif self.defense_technique == "krum":
                net_list, net_freq = self._defender.exec(client_models=net_list,
                                                        num_dps=[self.num_dps_poisoned_dataset]+num_data_points,
                                                        g_user_indices=g_user_indices,
                                                        device=self.device)
            elif self.defense_technique == "multi-krum":
                net_list, net_freq = self._defender.exec(client_models=net_list,
                                                        num_dps=[self.num_dps_poisoned_dataset]+num_data_points,
                                                        g_user_indices=g_user_indices,
                                                        device=self.device)
            elif self.defense_technique == "rfa":
                net_list, net_freq = self._defender.exec(client_models=net_list,
                                                        net_freq=net_freq,
                                                        maxiter=500,
                                                        eps=1e-5,
                                                        ftol=1e-7,
                                                        device=self.device)
            else:
                NotImplementedError("Unsupported defense method !")

            # after local training periods
            ############################################################################################################################################

            # net_avg is modified by average (fed_avg_aggregator)
            # net list has the updated (10 net) trained on 10 client's sampled images
            # net freq of len 10 is the ratio of # client's images to the total number of images in that round

            if self.defense_technique != "data-defense":
                self.net_avg = fed_avg_aggregator(net_list, net_freq, device=self.device, model=self.model)


            if self.defense_technique == "data-defense":

                paramNettest = {} #dictionary of parameters for Net_test
                for name, param in net_list[0].named_parameters():
                    paramNettest[name] = torch.from_numpy(np.zeros(param.shape, dtype='float32')).cuda()

                #filling parameters for Net_test with Net1's parameters
                for i in range(self.part_nets_per_round):
                    for name, param in net_list[i].named_parameters():
                        paramNettest[name] += net_freq[i]*param

                #Initiating Net_test class with filled parameters

                new_model = vgg9.Net_test(paramNettest)
                new_model = new_model.cuda()

                #### UPDATING THE GLOBAL MODEL'S PARAMETERS ########
                self.net_avg.load_state_dict(paramNettest)

                ########################################################################################################################################

                def poison_loss(output,target):
                    output = output - torch.max(output)
                    X_exp_one = torch.exp(output[target])
                    X_exp_all = torch.exp(output)
                    partition = X_exp_all.sum(0, keepdim=True)
                    softmax = X_exp_one / partition
                    if softmax==1.0:
                        l_ap = -1.0 * torch.log(1.0000001 - softmax)
                    else:
                        l_ap = -1.0 * torch.log(1.0 - softmax)
                    return l_ap

                def clean_loss(output,target):
                    output = output - torch.max(output)
                    X_exp_one = torch.exp(output[target])
                    X_exp_all = torch.exp(output)
                    partition = X_exp_all.sum(0, keepdim=True)
                    softmax = X_exp_one / partition
                    if softmax==0.0:
                        l_ac = -1.0 * torch.log(0.0000001 + softmax)
                    else:
                        l_ac = -1.0 * torch.log(softmax)
                    return l_ac

                new_model.eval()


                for data in poisoned_train_loader_flag:

                    #inputs, targets, flag, z = data
                    #inputs, targets, flag, z = inputs.cuda(), targets.cuda(), flag.cuda(), z.cuda()

                    inputs, targets, flag, gamma = data
                    inputs, targets, flag, gamma = inputs.cuda(), targets.cuda(), flag.cuda(), gamma.cuda()

                    outputs = new_model(inputs)
                    loss_ap = torch.tensor([0.],requires_grad=True).cuda()
                    loss_ac = torch.tensor([0.],requires_grad=True).cuda()
                    loss = torch.tensor([0.],requires_grad=True).cuda()
                    sum_value_loss = torch.tensor([0.],requires_grad=True).cuda()
                    value_fn = torch.tensor([0.],requires_grad=True).cuda()
                    count_ap = 0.0
                    count_ac = 0.0

                    for i in range(inputs.shape[0]):

                        if flag[i] == 1:
                            l_ap = poison_loss(outputs[i],targets[i])
                            loss_ap = loss_ap + l_ap
                            count_ap = count_ap+1
                            Value_l_ap = l_ap
                            Value_l_ac = clean_loss(outputs[i],targets[i])
                            #sum_value_loss += z[i]*Value_l_ap + (1-z[i])*Value_l_ac
                            #sum_value_loss += z[i] * (Value_l_ap - Value_l_ac)
                            sum_value_loss += gamma[i] * (Value_l_ap - Value_l_ac)

                        else:
                            l_ac = clean_loss(outputs[i],targets[i])
                            loss_ac = loss_ac + l_ac
                            count_ac = count_ac+1
                            Value_l_ac = l_ac
                            Value_l_ap = poison_loss(outputs[i],targets[i])
                            #sum_value_loss += z[i]*Value_l_ap + (1-z[i])*Value_l_ac
                            #sum_value_loss += z[i] * (Value_l_ap - Value_l_ac)
                            sum_value_loss += gamma[i] * (Value_l_ap - Value_l_ac)


                    if count_ac !=0:
                        loss_ac = loss_ac/count_ac

                    if count_ac ==0:
                        loss_ac = loss_ac*0.0

                    if count_ap !=0:
                        loss_ap = loss_ap/count_ap

                    if count_ap ==0:
                        loss_ap = loss_ap*0.0

                    value_fn = sum_value_loss/inputs.shape[0]

                    #alpha = 0.75
                    #loss = (alpha*loss_ac) + ((1-alpha)*loss_ap)

                    loss = loss_ac + loss_ap

                    print("Loss_ac = " ,loss_ac)
                    print("Loss_ap = " ,loss_ap)
                    print("Loss = " ,loss)
                    print("Value function = " ,value_fn)

                    config.theta.retain_grad()
                    config.w1.retain_grad()
                    config.w2.retain_grad()
                    config.w3.retain_grad()
                    config.w4.retain_grad()

                    '''
                    # L2 REGULARISER
                    l2_lambda = 0.01
                    l2_reg = torch.tensor(0.,requires_grad=True).cuda()
                    for param in new_model.parameters():
                        l2_reg += torch.norm(param)
                    loss += l2_lambda * l2_reg
                    '''

                    loss.backward(retain_graph=True)
                    with torch.no_grad():

                        #print("Theta before gradient descent = ",config.theta)
                        #print("Gradient of loss wrt theta = ", config.theta.grad)
                        config.theta.sub_(config.theta.grad*0.01)
                        #print("Theta after gradient descent = ",config.theta)
                        config.theta.grad.zero_()


                        for i in range(self.part_nets_per_round):
                            for name, param in net_list[i].named_parameters():
                                if param.grad is not None:
                                    if param.grad.grad_fn is not None:
                                        param.grad.detach_()
                                    else:
                                        param.grad.requires_grad_(False)
                                    param.grad.zero_()

                        for name, param in self.net_avg.named_parameters():
                            if param.grad is not None:
                                if param.grad.grad_fn is not None:
                                    param.grad.detach_()
                                else:
                                    param.grad.requires_grad_(False)
                                param.grad.zero_()

                        for name,param in new_model.named_parameters():
                            if param.grad is not None:
                                if param.grad.grad_fn is not None:
                                    param.grad.detach_()
                                else:
                                    param.grad.requires_grad_(False)
                                param.grad.zero_()


                      

                    value_fn.backward(retain_graph=True)

                    with torch.no_grad():

                        norm_W1_grad = torch.norm(config.w1.grad)
                        norm_W2_grad = torch.norm(config.w2.grad)
                        norm_W3_grad = torch.norm(config.w3.grad)
                        norm_W4_grad = torch.norm(config.w4.grad)
                        norm_grad_all = torch.tensor([norm_W1_grad, norm_W2_grad, norm_W3_grad, norm_W4_grad])
                        norm_psi = torch.norm(norm_grad_all)
                        psi_norm.append(norm_psi.item())


                        #print("w1, w2 before gradient descent = ",config.w1, config.w2)
                        #print("Gradient of value fn wrt w1,w2 = ", config.w1.grad, config.w2.grad)

                        '''
                        config.w1.sub_(config.w1.grad*self.gamma_lr)
                        config.w2.sub_(config.w2.grad*self.gamma_lr)
                        config.w3.sub_(config.w3.grad*self.gamma_lr)
                        config.w4.sub_(config.w4.grad*self.gamma_lr)
                        '''
                        config.w1.sub_(config.w1.grad*0.00001)
                        config.w2.sub_(config.w2.grad*0.00001)
                        config.w3.sub_(config.w3.grad*0.00001)
                        config.w4.sub_(config.w4.grad*0.00001)

                        #print("w1, w2 after gradient descent = ",config.w1, config.w2)
                        config.w1.grad.zero_()
                        config.w2.grad.zero_()
                        config.w3.grad.zero_()
                        config.w4.grad.zero_()

                        for i in range(self.part_nets_per_round):
                            for name, param in net_list[i].named_parameters():
                                if param.grad is not None:
                                    if param.grad.grad_fn is not None:
                                        param.grad.detach_()
                                    else:
                                        param.grad.requires_grad_(False)
                                    param.grad.zero_()

                        for name, param in self.net_avg.named_parameters():
                            if param.grad is not None:
                                if param.grad.grad_fn is not None:
                                    param.grad.detach_()
                                else:
                                    param.grad.requires_grad_(False)
                                param.grad.zero_()

                        for name,param in new_model.named_parameters():
                            if param.grad is not None:
                                if param.grad.grad_fn is not None:
                                    param.grad.detach_()
                                else:
                                    param.grad.requires_grad_(False)
                                param.grad.zero_()

                    loss1 = loss.item()
                    self.loss_total.append(loss1)

                    loss2 = loss_ac.item()
                    self.loss_clean.append(loss2)

                    loss3 = loss_ap.item()
                    self.loss_poison.append(loss3)

                    value1 = value_fn.item()
                    self.value_total.append(value1)

                    #del outputs, loss, loss_ac, loss_ap,value_fn, inputs, targets, flag, z, sum_value_loss
                    del outputs, loss, loss_ac, loss_ap,value_fn, inputs, targets, flag, gamma, sum_value_loss
                    torch.cuda.empty_cache()

                paramNettest.clear()
                del new_model

            #############################################################################################################################
            ######################################################################################################################################

            if self.defense_technique == "weak-dp":
                # add noise to self.net_avg
                noise_adder = AddNoise(stddev=self.stddev)
                noise_adder.exec(client_model=self.net_avg,
                                                device=self.device)
            
            # norm difference between the original global model and the average global model
            calc_norm_diff(gs_model=self.net_avg, vanilla_model=self.net_avg, epoch=0, fl_round=flr, mode="avg")
            
            logger.info("Measuring the accuracy of the averaged global model, FL round: {} ...".format(flr))
            
            # test on 10000 test images of cifar10
            # overall_acc means on 10000 test images, raw_acc means on 1000 truck images of cifar10 test set 
            overall_acc, raw_acc = test(self.net_avg, self.device, self.vanilla_emnist_test_loader, test_batch_size=self.test_batch_size, criterion=self.criterion, mode="raw-task_acc", dataset=self.dataset, poison_type=self.poison_type)
            
            
            # test on 196 southwest test images
            # backdoor_acc means test acc on 196 southwest test images (labeled as truck)
            backdoor_acc, _ = test(self.net_avg, self.device, self.targetted_task_test_loader, test_batch_size=self.test_batch_size, criterion=self.criterion, mode="targetted-task", dataset=self.dataset, poison_type=self.poison_type)
 
            # fl_iter_list is list of 0,1,2,...,500
            fl_iter_list.append(flr)
            # main task acc on 10000 test images of cifar10
            main_task_acc.append(overall_acc)
            # raw_task_acc on 1000 truck images of cifar10 test set
            raw_task_acc.append(raw_acc)
            # backdoor_acc on 196 southwest test images (labeled as truck)
            backdoor_task_acc.append(backdoor_acc)
			
            if len(current_adv_norm_diff_list) == 0:
                adv_norm_diff_list.append(0)
            else:
                # if you have multiple adversaries in a round, average their norm diff
                adv_norm_diff_list.append(1.0*sum(current_adv_norm_diff_list)/len(current_adv_norm_diff_list))

            ###############################################################
            if self.defense_technique == "data-defense":
                freq_clients = net_freq.tolist()
                freq_list.append(freq_clients)


            if self.defense_technique == "data-defense":
                df2 = pd.DataFrame({'freq_list': freq_list}) 
                freq_filename = "freq_assigned.csv"
                df2.to_csv(freq_filename, index=False)
                logger.info("Wrote weights results to: {}".format(freq_filename))
        ###################################################################


            df = pd.DataFrame({'fl_iter': fl_iter_list, 
                            'main_task_acc': main_task_acc, 
                            'backdoor_acc': backdoor_task_acc, 
                            'raw_task_acc':raw_task_acc, 
                            'adv_norm_diff': adv_norm_diff_list, 
                            'wg_norm': wg_norm_list
                            })
       
            if self.poison_type == 'ardis':
                # add a row showing initial accuracies
                df1 = pd.DataFrame({'fl_iter': [0], 'main_task_acc': [88], 'backdoor_acc': [11], 'raw_task_acc': [0], 'adv_norm_diff': [0], 'wg_norm': [0]})
                df = pd.concat([df1, df])

            results_filename = get_results_filename(self.poison_type, self.attack_method, self.model_replacement, self.project_frequency, self.defense_technique, self.norm_bound, self.prox_attack, False, self.model)

            df.to_csv(results_filename, index=False)
            logger.info("Wrote accuracy results to: {}".format(results_filename))


            if self.defense_technique == "data-defense":

                folder1 = f"./loss_learning"
                np.savez(f'{folder1}', loss_total = self.loss_total, loss_clean = self.loss_clean, loss_poison = self.loss_poison)

                np.save("value_total",self.value_total)

                np.save("gamma_values",gamma_values)

                np.save("poison_clean_indices",poison_clean_indices)

                np.save("ce_loss",ce_loss)

                np.save("red_top",red_top)
                np.save("green_top",green_top)
                np.save("red_rest",red_rest)
                np.save("green_rest",green_rest)

                np.save("psi_norm",psi_norm)

                if flr==1:
                    np.save("gamma_diff1",gamma_diff1)


            np.save("class_main_task_acc",class_main_task_acc)

            #folder6 = f"./class_backdoor_acc"
            #np.savez(f'{folder6}', class_backdoor_acc = class_backdoor_acc)

            # save model net_avg
            # torch.save(self.net_avg.state_dict(), "./checkpoint/emnist_lenet_10epoch.pt")
