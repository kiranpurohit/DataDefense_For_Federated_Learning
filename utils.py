...
def partition_data(dataset, datadir, partition, n_nets, alpha, args):
    if dataset == 'mnist':
        ...
        print('In triggerpatch of partition')
        X_train, y_train, X_test, y_test = load_cifar10_data(datadir)
        n_train = X_train.shape[0]
        #print('utils.py-> n_train: ', n_train)
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None):
    if dataset in ('mnist', 'emnist', 'cifar10','triggerpatch'):
        if dataset == 'mnist':
...
def load_poisoned_dataset(args):
    ...
    if args.dataset in ("mnist", "emnist"):
        ...
        if args.poison_type == "southwest":
            ...
            if args.attack_case == "edge-case":
                ...
                mislabeled_trainset.data = saved_southwest_dataset_train
                mislabeled_trainset.targets = sampled_targets_array_train
                
                #print("southwest images = ",saved_southwest_dataset_train[56],saved_southwest_dataset_train.shape)
                #print("southwest labels = ",sampled_targets_array_train[56], len(sampled_targets_array_train))
                
                #print("mislabelled images=",mislabeled_trainset.data[56],mislabeled_trainset.data.shape)
                #print("mislabelled labels = ",mislabeled_trainset.targets[56], mislabeled_trainset.targets.shape)
            ...
            # 500 images mix (100+400)              
            poisoned_train_loader = torch.utils.data.DataLoader(poisoned_trainset, batch_size=args.batch_size, shuffle=True)

            # 500 images (100+400) with batch size 500
            poisoned_train_loader_1 = torch.utils.data.DataLoader(poisoned_trainset, batch_size=500, shuffle=False)

            # 400 clean images of CIFAR10
            clean_train_loader = torch.utils.data.DataLoader(clean_trainset, batch_size=128, shuffle=True)

            # 100 poisoned images of southwest
            mislabeled_train_loader = torch.utils.data.DataLoader(mislabeled_trainset, batch_size=args.batch_size, shuffle=True)
           
            # 50000 train images of CIFAR10
    ...
    elif args.dataset == "triggerpatch":
        if args.poison_type == "label_flip":
            transform_train = transforms.Compose([
                transforms.RandomCrop(32, padding=4),
                transforms.RandomHorizontalFlip(),
                transforms.ToTensor(),
            ...
            if args.attack_case == "edge-case":         ### yha se train and test dataset utaya
                with open('./saved_datasets/784_train_trigger_car.pkl', 'rb') as train_f:
                    saved_patch_dataset_train = pickle.load(train_f)
                    print('saved_patch_dataset_train', len(saved_patch_dataset_train))

                with open('./saved_datasets/196_test_trigger_car.pkl', 'rb') as test_f:
                    saved_patch_dataset_test = pickle.load(test_f)
                    print('saved_patch_dataset_test', len(saved_patch_dataset_test))

            logger.info("OOD (Trigger Patch) train-data shape we collected: {}".format(saved_patch_dataset_train.shape))

            sampled_targets_array_train = 2 * np.ones((saved_patch_dataset_train.shape[0],), dtype =int) # car -> label as bird
            '''
            with open('./saved_datasets/784_train_trigger_label.pkl', 'rb') as train_l:
                    sampled_targets_array_train = pickle.load(train_l)
                    print('saved_patch_dataset_label_train', len(sampled_targets_array_train))
            sampled_targets_array_train = np.array(sampled_targets_array_train)
            '''
            sampled_targets_array_test = 2 * np.ones((saved_patch_dataset_test.shape[0],), dtype =int) # car -> label as bird
            '''
            with open('./saved_datasets/196_test_trigger_label.pkl', 'rb') as test_l:
                    sampled_targets_array_test = pickle.load(test_l)
                    print('saved_patch_dataset_label_test', len(sampled_targets_array_test))
            sampled_targets_array_test = np.array(sampled_targets_array_test)
            '''
            # downsample the poisoned dataset #################
            if args.attack_case == "edge-case":
                num_sampled_poisoned_data_points = 100 # N
                samped_poisoned_data_indices = np.random.choice(saved_patch_dataset_train.shape[0],
                                                                num_sampled_poisoned_data_points,
                                                                replace=False)
                saved_patch_dataset_train = saved_patch_dataset_train[samped_poisoned_data_indices, :, :, :]
                sampled_targets_array_train = np.array(sampled_targets_array_train)[samped_poisoned_data_indices]

                mislabeled_trainset.data = saved_patch_dataset_train
                mislabeled_trainset.targets = sampled_targets_array_train
            ...
            poisoned_trainset.data = np.append(poisoned_trainset.data, saved_patch_dataset_train, axis=0)
            poisoned_trainset.targets = np.append(poisoned_trainset.targets, sampled_targets_array_train, axis=0)

            logger.info("{}".format(poisoned_trainset.data.shape))      ### 32x32 , 500 image, 3 
            logger.info("{}".format(poisoned_trainset.targets.shape))
            logger.info("{}".format(sum(poisoned_trainset.targets)))        ### targets ka sum
            ...
            poisoned_testset = copy.deepcopy(testset)
            poisoned_testset.data = saved_patch_dataset_test
            poisoned_testset.targets = sampled_targets_array_test
            ...
            '''
            print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
            print('poisoned_trainset length: ', poisoned_trainset.data.shape)
            print('posioned_train_loader: ', poisoned_train_loader)
            print('trainset length: ', trainset.data.shape)
            print('trainloader: ', trainloader)
            print('Clean_trainset: ', clean_trainset.data.shape)
            print('clean_train_loader: ', clean_train_loader)
            print('testset length: ', testset.data.shape)
            print('vanilla_test_loader: ', vanilla_test_loader)
            print('posioned_testset length: ', poisoned_testset.data.shape)
            print('targetted_task_test_loader: ', targetted_task_test_loader)
            print('nums_dps_poisoned_dataset: ', num_dps_poisoned_dataset)
            print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%') 
            '''



    return poisoned_train_loader, vanilla_test_loader, targetted_task_test_loader, num_dps_poisoned_dataset, clean_train_loader, mislabeled_train_loader, poisoned_trainset, poisoned_train_loader_1 
